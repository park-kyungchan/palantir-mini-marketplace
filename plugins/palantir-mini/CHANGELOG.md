...
- **G-HOOK-O — non-conforming PreToolUse hook output shape**: a bare top-level `decision: "continue"` (invalid for Claude Code's real PreToolUse contract — only `hookSpecificOutput.permissionDecision: "allow"|"deny"` or omitting `decision` entirely is valid for a non-block verdict) was emitted by 7 production hooks, not just the single scout-confirmed offender: `hooks/ontology-engineering-workflow-enforcement-gate.ts`, `hooks/gates/ontology-engineering-workflow-enforcement-gate.impl.ts`, `hooks/prompt-dtc-enforcement-gate.ts`, `hooks/ontology-import-guard.ts`, `hooks/write-scope-runtime-enforce.ts`, `hooks/researcher-citation-precision.ts`, `hooks/value-grade-assigner.ts`, plus `hooks/semantic-frontmatter-validate.ts` (found during the same pass — same anti-pattern). Fix is subtractive only: the top-level `decision: "continue"` key is deleted from every non-conforming return site; `decision: "block"` sites (already paired with `hookSpecificOutput.permissionDecision: "deny"`) are untouched. Each touched file's local `HookResult`-shaped type is narrowed (`decision?: "continue" | "block"` → `decision?: "block"`, or the field dropped entirely where the hook never blocks) so the type system enforces the invariant going forward. `schemas/hooks/governance-hook.output.schema.json`'s `decision` enum is narrowed from `["block", "continue"]` to `["block"]` (the root cause that let the wrong runtime shape pass metadata-only conformance checks all session). New table-driven regression test `tests/hooks/pretooluse-output-shape-conformance.test.ts` walks every `type: "command"` entry under `hooks.json`'s `PreToolUse` block, spawns each with a benign `Read` payload, and asserts any emitted `decision` is `"block"` (paired with `permissionDecision: "deny"`) or absent — so a future non-conforming hook fails CI instead of only this one. Test-assertion sites across 5 existing test files (`.decision).toBe("continue")` → `.toBeUndefined()`) updated to match.
- **Test-home isolation for the prompt-DTC gate test suite**: 7 test files that call `promptDtcEnforcementGate`/`.impl` directly wrote to the developer's real `~/.palantir-mini` global state directory (via `advisory-shown-store.ts`/`delivery-grant-store.ts`/`global-session-index.ts`, all `PALANTIR_MINI_GLOBAL_STATE_DIR`-overridable) instead of an isolated tmp dir: `tests/hooks/prompt-dtc-enforcement-gate.test.ts`, `tests/hooks/prompt-dtc-enforcement-gate-dtc-turn.test.ts`, `tests/hooks/prompt-dtc-scoped-blocking.test.ts`, `tests/hooks/prompt-dtc-enforcement-gate-fde-skip.test.ts`, `tests/hooks/prompt-dtc-enforcement-gate-authorized-delivery.test.ts`, `tests/evals/prompt-to-dtc-regression.test.ts`, `tests/e2e/drift-rebind-altitude-replay.test.ts`. Each now sets `process.env.PALANTIR_MINI_GLOBAL_STATE_DIR` to a fresh `fs.mkdtempSync` dir in `beforeEach` (cleaned up in `afterEach` alongside its existing tmp-dir sweep), following the reference idiom already proven in `tests/hooks/prompt-dtc-enforcement-gate-advisory-dedup.test.ts`. New standalone guard `scripts/verify-real-home-untouched.ts` (`bun run test:home-isolation-guard`) snapshots the real `~/.palantir-mini` before/after running the 7 fixed files as a subprocess and exits non-zero on any diff — a manual/CI-optional check, deliberately **not** wired into the default `bun test`/CI invocation (a real-home ambient-state dependency in the default suite would be the opposite of the isolation this fix establishes).
- **Review-found residual real-home leak in `scripts/log.ts`'s `resolveEmitRoot()`**: the scoped fix above only isolated the 7 named test files; a full unscoped `bun test` run still wrote new files under the real `~/.palantir-mini/session/` (events.jsonl appends + `outcome-pairs/*.json`). Root cause: when a handler-supplied `cwd` is ALREADY an excluded root (real `HOME`, satisfied by `HOME`'s own genuine `.palantir-mini` marker), `resolveEmitRoot()`'s "found root is excluded → fall back to `startDir`" branch returned `startDir`, which in this case IS the same excluded dir — the exclusion check was a no-op whenever `cwd === HOME` exactly. Fixed by also checking `isExcludedProjectRoot(startDir)` in that fallback and routing to a dedicated `os.tmpdir()`-rooted scratch dir instead of `startDir` when it fires. Re-verified: a full unscoped `bun test` run now leaves the real `~/.palantir-mini` tree with zero new/modified files (mtime-identical file listing before/after).

### Changed

- **G-SKILL-P slice A — skill-catalog description token diet**: re-ran `scripts/slim-skill-frontmatter.ts` (existing `MAX_DESCRIPTION_CHARS = 88` policy, no code change) over the 7 skills whose `description:` frontmatter exceeded the cap: `pm-eval-suite`, `pm-research`, `pm-ontology-drift-propose`, `pm-value-audit`, `pm-cold-start-orchestrate`, `pm-impact-quick`, `pm-walk-analyze`. Measured reduction: 253 raw description chars (852 → 599) across the 7 files, ≈63 tokens saved per agent spawn when the skill catalog is injected. `pm-research`'s mechanical output was hand-overridden (the 88-char boundary fell mid-list and would have silently dropped the "audit" mode name, one of its 3 named trigger modes) to `"Research-library lifecycle: diff | refresh | audit modes (drift, re-fetch, staleness)."` (86 chars) — keeps all 3 mode names inside budget. `bun run scripts/slim-skill-frontmatter.ts --dry-run` reports `changedFiles=0` after landing (idempotent). Only `description:` frontmatter lines changed — no skill body/instruction drift. G-SKILL-P slice B (remove/merge candidates) remains a separate USER-GATED decision, not executed in this PR — see the pending-ledger row.

### Not resolved — G-DEAD-Q disposition correction (design assumption overridden by direct re-verification)

- `~/projects/_workspace/2026-07-12-w10-closure/design-w5-token-diet.md` §Slice 5 directed closing G-DEAD-Q as `RESOLVED-STALE`, on the premise (via `scout-w5-hooks.md`'s grep-based recount) that all 4 `collapsibleBoilerplateLines` call sites in `hooks/gates/prompt-dtc-enforcement-gate.impl.ts` are reachable. Direct execution of `promptDtcEnforcementGate` during this landing (not grep/reading alone) found that premise wrong: the `scoped-advisory` and `sic-miss` sites are **structurally unreachable** under the current `PROJECT_GATE_POLICY_MINIMUMS` floor table (`lib/governance/effective-gate-mode.ts`) — every classified mutation's floor strengthens `mode` past `selective-blocking` before the dispatch that guards those two branches is ever reached. This PR ships **no code change** for G-DEAD-Q (per the design's own "no code change" instruction) and does **not** close the ledger row `RESOLVED-STALE` — the row's disposition is `REOPENED (not closeable this PR)`, with the full re-verification trace in `_workspace/pm-pending-ledger.md`. Flagged here explicitly because the design doc's own closing instruction is now stale against what actually shipped — a future reader should trust the ledger row, not the design doc's Slice 5 text, for G-DEAD-Q's status.

## [7.50.0] - 2026-07-12 — session-standing delivery approval (G-RPLY-M) + advisory boilerplate dedup (G-ADV-N)

Grounds: `g12 de-2026-07-11-authorization-flexibility-over-phrase-markers` (standing authorization-flexibility posture) + `g12 de-2026-07-12-authorization-friction-corrective-user-2026-07-12-recurring-per-delivery` (this session's superseding USER directive — revises the 30-min absolute TTL clause of `g12 de-2026-07-12-pm-flex-slices-2-3-policy-answers-locked` and directs the advisory-boilerplate token diet).

### Fixed

- **G-RPLY-M — the approval anti-replay window slid past the genuine approval envelope on interleaved system-notification prompts**: `hooks/prompt-front-door-capture.ts` had zero discrimination between a real user turn and a system/task-notification-shaped `UserPromptSubmit` payload, so every non-empty captured prompt (including notification-shaped ones) advanced the SAME `current` pointer file `coreVerify` check #5 anchors against — a session with several notification-shaped turns between a genuine approval and its consuming delivery call could slide the recency window past the approving envelope. New `lib/prompt-front-door/prompt-origin-classifier.ts` (`classifyPromptOrigin`, fail-open structural + operator-configured heuristics) gates a SECOND, narrower `current-user-authored` pointer (`lib/prompt-front-door/store.ts`'s `writeLastUserAuthoredPointer`/`readLastUserAuthoredPointer`) that only advances on user-authored turns; `verifyDeliveryApprovalAgainstEnvelope` (`lib/lead-intent/dtc-build-approval.ts`) now anchors check #5 against this pointer instead of the general one, which every OTHER consumer (turn continuity, the DTC-build dispatch lane) keeps reading unchanged. The `pm_authorize_delivery` grant (`lib/prompt-front-door/delivery-grant-store.ts`) is made SESSION-STANDING: `DELIVERY_GRANT_TTL_MS` (30 min) is renamed `DELIVERY_GRANT_SAFETY_NET_TTL_MS` (24 h, a cross-session safety net only — a new Claude Code session mints a new `sessionId` and therefore a new grant file, so cross-session isolation was already structural and independent of the TTL value), with an explicit `revokeDeliveryGrant`/`revokeDeliveryGrantSync` surface as the real "end this grant early" control, driven by a bilingual (ko/en) user-authored revoke phrase (`excerptRequestsDeliveryGrantRevocation`) wired into the capture hook. `EnvelopeStore`'s widened interface, the doc-string corrections across `bridge/mcp-server.ts`'s `pm_authorize_delivery` tool description, the gate's delivery-escape paragraph, `lib/event-log/types.ts`, and `runtime-overlay/schemas-snapshot/ontology/lineage/event-types.ts` (schemas-snapshot 1.97.1 -> 1.97.2, doc-only PATCH, no primitive type/field/export change) all land in this same fix.

### Added

- **G-ADV-N — session-scoped advisory boilerplate dedup**: `hooks/gates/prompt-dtc-enforcement-gate.impl.ts`'s scoped-advisory, SIC-cache-miss, and generic/delivery-blocking branches re-emitted several byte-identical filler lines ("Allowed while pending: ...", "Pass condition: ...", the plain `MODE=off` escape line) on every matching mutating call with no per-session dedup, at an estimated few hundred tokens per repeated occurrence — both for the Lead session and for every dispatched subagent sharing its `session_id`. New `lib/prompt-front-door/advisory-shown-store.ts` (`hasAdvisoryBoilerplateBeenShown`/`markAdvisoryBoilerplateShown`, GLOBAL home-level path keyed by `(runtime, sessionId)`, same proven shape as `delivery-grant-store.ts`, `PALANTIR_MINI_GLOBAL_STATE_DIR`-overridable for tests, fail-OPEN on any store error since this is a token-cost optimization, never a security control) backs a new `collapsibleBoilerplateLines` helper wired into all three call sites: the four identified filler lines collapse to a `<=10`-token marker (`[boilerplate: see earlier this session]`) after the first occurrence per `(runtime, sessionId, boilerplateClass)` this session. `assessment.reason` (the call-varying diagnostic), `runbookPointerForAssessment(...)` (varies by `errorClass`), and the `pm_authorize_delivery` authorize-delivery paragraph (the ONLY place the unblocking mechanism is documented) are NEVER collapsed — they stay full on every single occurrence, per G-ADV-N's requirement that a genuine blocking event always carries full actionable text.
...
- **G-GATE-J — `gh pr merge`-class delivery proof measured the WRONG ground-truth object**: `hooks/gates/prompt-dtc-enforcement-gate.impl.ts`'s `protectedMutationClassForPromptGate` and `isProvenNonOntologyDelivery` both proved a `gh pr` subcommand's change-set via `allPushRangePathsNonOntology` (the local push range, `git diff origin/<branch>...HEAD`) for EVERY subcommand including `merge/close/reopen/edit/ready/review`. Those subcommands operate on an EXISTING PR, whose change-set ground truth is the PR's OWN diff — the local push range is EMPTY (conservatively `false`, over-blocking) whenever the calling session's HEAD is already synced with its upstream, exactly the case a `gh pr merge` run from a synced `main` hits (empirically confirmed on both the marketplace and home repos). The A2 grant lane was therefore unreachable for any `gh pr merge`, regardless of a live, correctly-minted `pm_authorize_delivery` grant. New shared helper `ghPrChangeSetNonOntology` (delegating to new `resolveGhPrDiffPaths`) is now the ONE mechanism both call sites use, so they cannot drift: `create` (no existing PR yet) keeps the original `allPushRangePathsNonOntology` semantics byte-identical; every other subcommand proves its change-set via `gh pr diff --name-only [selector] [-R <repo>]`, parsing the selector (bare PR number/URL/branch, or absent for the current branch) and an optional `-R`/`--repo` flag out of the raw command text. `resolveGhPrDiffPaths` uses `execFileSync` (argv array, no shell) deliberately — building a shell string from a model-parsed selector token would open a SECOND, hook-internal command-injection surface, executed regardless of the real tool call's eventual allow/deny outcome. Fail-closed throughout: no subcommand match, a thrown `gh` invocation (missing binary, auth failure, network error), or empty stdout all return `undefined`, keeping the conservative BLOCKING floor — the cost of a `gh` failure in the hook is delivery-call-only latency, never a false non-ontology proof.
- **G-DOCS-K — docs-class audit-after carve-out for the A2 delivery-proof predicate** (USER directive 2026-07-12): new predicate `isDocsClassAuditAfterPath` (a lowercased `.md`-suffixed path that is not an ontology schema surface `schemas/ontology/` and not under the live `.palantir-mini/` ontology-state dir) wired into `isNonOntologyPath` ONLY, checked after the two existing hard floors and before `pathIsProjectOntologyClass` — so a doc literally named e.g. `ontology-notes.md`, or living under `projects/<p>/ontology/`, is no longer swept into the `pull-request`/`commit`/`release` BLOCKING floor merely for containing an `ontology/`-segment-shaped substring. `scopedBlockingFileReason` (the edit-time/advisory surface consumed by `Edit`/`Write`/`commit_edits` gating) is BYTE-IDENTICAL — the carve applies ONLY to this delivery-proof predicate. By design (R3, not a bug): a delivery whose change-set contains a TRUE (non-docs-class) ontology surface keeps its blocking floor even with a live grant — grants never relax the A1 ontology floor.
- New tests: `tests/hooks/prompt-dtc-enforcement-gate-gh-pr-diff-ground-truth.test.ts` — (a) `gh pr merge` from a synced-main fixture with a faked docs/governance PR diff resolves `generic-mutation` + `isProvenNonOntologyDelivery` true; (b) a faked PR diff containing an ontology path stays `pull-request` + proof false (R3); (c) a `gh` failure/network-error keeps the conservative floor; (d1-d5) the docs-class carve's `isNonOntologyPath`/`isDocsClassAuditAfterPath` unit behavior plus a direct assertion that `scopedBlockingFileReason` is unaffected; (e) `gh pr create` stays on the unchanged push-range path. Updated the existing `tests/hooks/prompt-dtc-enforcement-gate-authorized-delivery.test.ts` and `tests/hooks/prompt-dtc-enforcement-gate-authorized-delivery-grant.test.ts` fixtures with a fake-`gh` PATH shim (`withFakeGhOnPath`) so their pre-existing `gh pr merge` PASS-WITH-AUDIT regression cases prove the SAME non-ontology change-set the legacy push-range mechanism used to resolve, now via `gh pr diff` instead.

## [7.46.0] - 2026-07-12 — pm authorization-flexibility slice 3 (design): G-DSN-E structured grant mechanism (subsumes G-ENV-B resolution)

Grounds: USER directive `g12 de-2026-07-11-authorization-flexibility-over-phrase-markers` + `g12 de-2026-07-12-pm-flex-slices-2-3-policy-answers-locked` (LOCKED: standalone `pm_authorize_delivery` MCP tool; SESSION scope; 30-min TTL within the locked 15-30min envelope; GLOBAL store; PR-scoped variant deferred) + `_workspace/2026-07-12-pmflex/context/design.json` slice 3 (rows `G-DSN-E`, `G-ENV-B`).

### Added

- **`pm_authorize_delivery` — new standalone MCP tool** (`bridge/handlers/pm-authorize-delivery.ts`): mints a SESSION-scoped delivery-authorization grant (30-minute absolute TTL, new explicit `DELIVERY_GRANT_TTL_MS` constant — `DTC_BUILD_APPROVAL_TTL_MS` in `lib/lead-intent/dtc-build-approval.ts` stays UNTOUCHED at 15 minutes for its own lane) from a re-verified user-approval envelope. This is a PURE surface-relocation of the existing A2 delivery-approval verification: it calls `verifyDeliveryApprovalAgainstEnvelope` VERBATIM (never reimplemented or forked). Resolves the current prompt-front-door envelope mirroring `pm-ontology-engineering-workflow.ts`'s `drift_rebind` seam (`readCurrentDriftRebindEnvelope`: explicit `(sessionId, promptId)` pair, else current-pointer walk per runtime), PLUS slice 2's global cross-lane fallback (`readGlobalSessionPointer`) so a dispatched subagent or second repo sharing the session can still resolve an envelope captured elsewhere. The grant is keyed by the RESOLVED envelope's `(runtime, sessionId)` — never raw caller input. No grant is ever written on a failed verification; the caller-supplied fields mirror the existing A2 `assess` call's field set (`userApprovalQuote` required, `userApprovalPromptId`/`userApprovalPromptHash` optional anchors, `promptId`/`promptHash`/`sessionId`/`runtime` hints). Registered in `bridge/mcp-server.ts` (`TOOLS[]` + `TOOL_CATEGORIES.lead-routing` + `HANDLER_MODULES`) and visible under the `altitude-2` MCP profile via `lib/capability-registry/mcp-tool-capability.ts`'s `STUDIO_CORE_MCP_TOOL_NAMES` (everyday-operation surface, per the g12 ruling that a hidden authorization tool is useless).
- **G-DSN-E — resolves G-ENV-B (A2 tool_input re-issue lane dead on Claude-native harness)**: `additionalProperties:false` on native Bash/Edit/Write tool schemas rejects the plugin-invented `userApprovalQuote`/`userApprovalPromptId`/`userApprovalPromptHash` fields before hooks ever run — a structural/platform constraint the plugin cannot patch on the native-tool side. `pm_authorize_delivery` is the fix: a plugin-owned MCP tool call BEFORE the gated native-tool call, whose result is looked up by the PreToolUse gate from disk state (the new grant store), not from new tool_input fields on the native call.
- **New GLOBAL delivery-grant store** (`lib/prompt-front-door/delivery-grant-store.ts`): a small pointer store at `<os.homedir()>/.palantir-mini/session/delivery-grants` (path overridable via `PALANTIR_MINI_GLOBAL_STATE_DIR`, same convention as slice 2's `global-session-index.ts`), keyed purely by `(runtime, sessionId)` — NOT per-project — so a dispatched subagent or second repo can consume a grant minted from ANY project root (spike-confirmed in slice 2: a subagent's PreToolUse `payload.session_id` equals the parent session's). Exports `issueDeliveryGrant`/`issueDeliveryGrantSync` (atomic upsert-mint), `readDeliveryGrant` (returns null for missing AND treats an expired-at-read grant as a miss; non-ENOENT errors are rethrown so consumption call sites fail closed via try/catch, mirroring `global-session-index.ts`'s error-handling convention), and `isDeliveryGrantLive` (the reusable expiry-check helper). **No revocation in v1** — TTL-only expiry: a later capture/turn superseding the current-pointer envelope does NOT revoke an already-issued session grant (that mid-turn-supersede volatility is exactly what G-DSN-E proof (i) showed was broken about strict turn-scoping).
- **Gate consumption**: `hooks/gates/prompt-dtc-enforcement-gate.impl.ts`'s A2 authorized-delivery branch now checks the GLOBAL delivery-grant store FIRST (`hasLiveDeliveryGrantForGate`, iterating the detected/preferred runtime then the rest of `PROMPT_RUNTIMES` since a plain native-tool payload rarely carries a runtime hint) — a live grant authorizes the delivery action WITHOUT requiring the caller to re-issue the tool call with `userApproval*` fields. On any miss/expiry/store-error, the gate falls through UNCHANGED to the existing tool_input re-issue lane (`assessDeliveryApprovalForGate`) and the phrase-marker/lexicon paths — purely additive, byte-identical fallback. The deny hint text (`hooks/gates/prompt-dtc-enforcement-gate.impl.ts`, the `isDeliveryClassBlocker` escape line) now presents `pm_authorize_delivery` as the primary authorization path (naming the tool + required fields + the 30-min session grant), with a secondary mention that runtimes without an MCP surface (Codex) can still use the existing re-issue/lexicon lanes. `verifyDeliveryApprovalAgainstEnvelope`'s tool_input acceptance code and the `excerptExpressesDtcBuildApproval`/`excerptExpressesDeliveryApproval` lexicons stay functional and byte-identical (fallback lexicons retained per the USER directive: demoted to fallback, not deleted).
- **New event type `delivery_authorization_granted`** (registry 84→85), mirroring the `drift_rebind_envelope_advanced` precedent exactly across the four surfaces: `lib/event-log/types.ts` (`DeliveryAuthorizationGrantedPayload` + envelope variant + union entry + `isDeliveryAuthorizationGranted` guard + optional `EventSnapshot` count), `runtime-overlay/schemas-snapshot/ontology/lineage/event-types.ts` (`EVENT_TYPE_NAMES` + `EVENT_TYPE_REGISTRY`, schemas package `1.96.0 → 1.97.0` + its own CHANGELOG entry), `runtime-overlay/schemas-snapshot/ontology/self/event-envelope.objecttype.ts` (`EVENT_ENVELOPE_INSTANCES` discriminator entry, count 84→85), `tests/ontology/self/event-envelope-registration.test.ts` (`EXPECTED_EVENT_ENVELOPE_COUNT` 84→85). Payload: `{ grantId, scope, sessionId, projectRoot, promptId, promptHash, issuedAt, expiresAt }`. `byWhom.identity: "user"` (carried-forward user approval, `drift_rebind_envelope_advanced` convention) — emitted ONLY on successful grant issuance; a failed verification emits no event and writes no grant. `lib/event-log/read/fold-snapshot.ts` gains the matching exhaustiveness switch case.
- **McpTool self-Ontology** (`runtime-overlay/schemas-snapshot/ontology/self/mcp-tool.objecttype.ts`) advances 23 → 24 (`pm_authorize_delivery` seeded), with paired drift-guard updates in `tests/ontology/self/executor-registration.test.ts` (`EXPECTED_MCP_TOOL_COUNT` 23→24) and a regenerated `tests/ontology/self/mcp-tool-fingerprint.golden.json` baseline. `cartography/TOOLS.md` regenerated (24 tools); `CARTOGRAPHY.md`'s altitude-2 visibility note updated 11/23 → 12/24.
- New tests: `tests/lib/prompt-front-door/delivery-grant-store.test.ts` (issue/read round-trip, TTL expiry-as-miss, upsert overwrite, per-key isolation, env-override rooting), `tests/bridge/handlers/pm-authorize-delivery.test.ts` (schema validation, fail-closed on a forged quote with no grant written, success mints a grant + emits the event with `byWhom.identity: "user"`, cross-lane envelope resolution mints the grant under the RESOLVED root not the caller's raw `project` hint), `tests/hooks/prompt-dtc-enforcement-gate-authorized-delivery-grant.test.ts` (live grant authorizes without re-issue fields, an expired grant falls through unchanged to the existing re-issue lane, an absent grant leaves today's denial byte-identical, cross-lane grant issued under root A honored for a gate call from root B).

## [7.45.0] - 2026-07-12 — pm authorization-flexibility slice 2 (design): G-ENV-A cross-lane envelope resolution (global session index)

Grounds: USER directive `g12 de-2026-07-11-authorization-flexibility-over-phrase-markers` + `g12 de-2026-07-12-pm-flex-slices-2-3-policy-answers-locked` (grant/envelope store = GLOBAL home-level, trust-boundary widening approved at 1-person scale) + `_workspace/2026-07-12-pmflex/context/design.json` slice 2 + spike result `_workspace/2026-07-12-pmflex/reports/spike-session-id.json` (a Task/Agent-dispatched subagent's PreToolUse `payload.session_id` EQUALS the parent session's session_id on claude CLI 2.1.207, so `(runtime, sessionId)` alone is a sufficient key — no parent-pointer chain needed).

### Added
- **G-ENV-A — cross-lane envelope/grant resolution via a GLOBAL session index**: new module `lib/prompt-front-door/global-session-index.ts` — a small pointer store at `<os.homedir()>/.palantir-mini/session/prompt-front-door-global` (path overridable via `PALANTIR_MINI_GLOBAL_STATE_DIR` for hermetic tests), keyed purely by `(runtime, sessionId)`. It is a REDIRECT, not an envelope copy: envelope authority stays single-copy in the local per-project `PromptFrontDoorStore` (`lib/prompt-front-door/store.ts`); the global index only points at the project root whose local store owns the authoritative envelope, closing the cross-lane miss a dispatched subagent or second repo (different cwd, same session) previously hit.
- **Capture dual-write**: `hooks/prompt-front-door-capture.ts`'s `promptFrontDoorCapture` best-effort upserts the global pointer for `(runtime, sessionId) -> projectRoot` immediately after the existing local write succeeds (`writePromptCaptureSync` itself is unchanged). Wrapped in try/catch — a global-index write failure never breaks or alters local capture.
- **Gate fallback**: `hooks/gates/prompt-dtc-enforcement-gate.impl.ts`'s `readCurrentEnvelope` now consults the global index ONLY when the local per-project lookup misses. On a redirect to a DIFFERENT project root, the envelope (and, since a redirected envelope's SIC/DTC contract records live under that SAME root, the contract-record reads in `assessPromptDtc`) resolve from the redirect target's own local store via the exact same store machinery a local hit would use. If the pointer targets the same root that already missed, or the redirect read also misses, the gate behaves exactly as today's miss (fail-closed). This only widens WHERE an envelope is found, never WHAT it authorizes — no approval/TTL/verification logic changed. The local per-project lookup remains the PRIMARY path and is byte-identical when it hits.
- New tests: `tests/lib/prompt-front-door/global-session-index.test.ts` (module round-trip, upsert-overwrite, per-key isolation, env-override rooting) and `tests/hooks/prompt-dtc-enforcement-gate-cross-lane.test.ts` (cross-lane redirect end-to-end pass, local-hit precedence over a poisoned global pointer, global-write failure leaves local capture intact, fail-closed when the redirect target has no envelope).

## [7.44.0] - 2026-07-12 — pm authorization-flexibility slice 1 (mechanical): idempotent drift_rebind, quote-aware delivery classifier, stale comment fix

Grounds: USER directive `g12 de-2026-07-11-authorization-flexibility-over-phrase-markers` + pending-ledger `_workspace/pm-pending-ledger.md` § 2026-07-11 rows `G-ENV-C`, `G-CLS-D`, `COMMENT-83` (slice 1 of the pm authorization-flexibility wave; slices 2-3 — cross-lane envelope/grant resolution and the structured grant mechanism — are separate, later slices per `_workspace/2026-07-12-pmflex/context/design.json`).

### Fixed
- **G-ENV-C — `drift_rebind` throws on an already-approved envelope**: `lib/prompt-front-door/rebind-persisted-approval.ts`'s `rebindPersistedApprovalToCurrentEnvelope` now short-circuits BEFORE the FSM walk / any write when `currentEnvelope.state === "digital_twin_approved"` — a duplicate/retried `drift_rebind` call (or an envelope already advanced by a prior turn) previously hit the FSM's missing `digital_twin_approved → semantic_intent_user_review` transition and threw. The existing `digitalTwinApprovalRef` is compared against the incoming (persisted, carried-forward) DTC's `approvalRef` via the already-exported `approvalRefToString` (`lib/prompt-front-door/approval-ref.ts`): identical → true no-op success (no write, no FSM walk, idempotent on repeat calls); DIFFERENT → fails closed with a distinct error ("current envelope already digital_twin_approved under a DIFFERENT approvalRef; refusing silent overwrite") instead of the prior generic FSM-transition error, preserving fail-closed security while fixing the idempotent-retry case. New tests in `tests/lib/prompt-front-door/rebind-persisted-approval.test.ts`: repeated rebind against the same sic/dtc is a no-op across 3 consecutive calls; rebind against an already-approved envelope under a genuinely different approvalRef throws the new distinct error.
- **G-CLS-D — delivery-classifier prose false-positive**: `hooks/gates/prompt-dtc-enforcement-gate.impl.ts`'s `protectedMutationClassForPromptGate` and `isProvenNonOntologyDelivery` regex-scanned the FULL lowercased Bash command string for delivery verbs (`gh pr create|merge|…`, `git push`, `npm publish`, `bun publish`, `release`, `deploy`) with no positional/argv anchoring, so a verb sitting inside a QUOTED, purely descriptive argument (e.g. a `decisions:emit --decision "ship the release"` governance-ledger append) misclassified as a real delivery action even though the executable performed none — empirically hard-blocked in the 2026-07-11 session until the prose was neutralized. New helper `stripQuotedSegments` (`lib/hooks/command-text.ts`) blanks the contents of quoted substrings before the verb regexes run, applied at both call sites. `isGitSubcommand` (already argv[0]-anchored) is unaffected and keeps scanning the raw command. SCOPE NUANCE preserved: `stripQuotedSegments` detects interpreter "execute a quoted script" idioms (`bash -c "…"`, `sh -c "…"`, `bun -e "…"`, `node -e "…"`, `python -c "…"`) and leaves the command UNSTRIPPED in that case, so a real delivery command hidden inside `bash -c "git push origin main"` still classifies — a naive "strip everything in quotes" would have traded the false-positive-block bug for a false-negative-allow bug. New table-driven fixtures in `tests/hooks/prompt-dtc-enforcement-gate.test.ts` (`describe("G-CLS-D quote-aware delivery classifier")`): 4 quoted-prose commands (including the decisions:emit ledger-shaped incident) that must NOT classify as a delivery class, and 4 real delivery commands (bare `git push`, `bash -c "git push origin main"`, bare `gh pr merge`, and a mixed command with quoted prose AND a real unquoted `git push`) that must STILL classify.
- **COMMENT-83 — stale "83 names" comment**: `tests/ontology/self/event-envelope-registration.test.ts` — the drift-guard comment referenced the pre-`cartography_decision_mirrored` count ("83 names") while the constant it describes (`EXPECTED_EVENT_ENVELOPE_COUNT = 84`) was already correct; comment now reads "84 names". Prose-only, zero behavioral change.

## [7.43.0] - 2026-07-11 — P1 unification M1 COMPLETE: S3 canonical grader + S2 mirrored-decision event type + emit-cli contract surface + doc salvage (schemas 1.96.0)

### Added — M1b completion (the tasks the two honest-stop sections below record as blocked; unblocked via the FULL governed SIC/DTC ceremony run through pm's own tools in the executor session — FDE start → approve_sic (genuine user-approved nine-axis SIC re-registered, Q2-verified) → 7-turn ontology-dtc-build fill (verdict dtc-filled) → Stage-06 inline envelope advance (digital_twin_approved, user approvalRefs carried forward) → work-contract + decision-record turn (workflow mutation-authorized) → 5-domain DTC review closure; zero gate bypasses, zero MODE=off, zero Bash-writes to gated files)
- **S3 — `runtime-overlay/schemas-snapshot/ontology/lineage/value-grade-grading.ts`** (schemas v1.96.0): the rule-26 T0..T4 `autoGradeEnvelope` lifted VERBATIM from `bridge/handlers/emit-event.ts` with a structural `GradeableEnvelope` input view (module imports only sibling package primitives). Function body diffed byte-identical to the main blob; 16-fixture old-vs-new differential harness identical=ALL (incl. the D2-after-C ordering edge + empty-`lineageRefs` edge), re-run and passing against the LANDED module. `bridge/handlers/emit-event.ts` now re-imports + RE-EXPORTS it (single grading source); importers `scripts/log.ts`, `hooks/value-grade-assigner.ts`, and the rule-26 conformance test resolve through the re-export unchanged. New exports subpath `./ontology/lineage/value-grade-grading`. New consumer test `tests/schemas/value-grade-grading.test.ts` (16 cases, every tier transition).
- **S2 — `cartography_decision_mirrored` event type** (registry 83→84): typed mirror of a home-cartography g12 DecisionEvent ledger row into events.jsonl (payload `{ sourceEventId, sourceLedger, sourceAtopWhich, decision, reasoning, expectedOutcome, memoryLayers, standing?, supersedes?, pairs?, intent?, outcomeRef?, mirroredBy: 'live-emit'|'backfill' }`; the g12 path-array atopWhich rides in `payload.sourceAtopWhich`, the envelope's own `atopWhich` stays CommitSha). Wired surface-by-surface on the `923666e` convention: `EVENT_TYPE_NAMES` + `EVENT_TYPE_REGISTRY`, `lib/event-log/types.ts` variant + `isCartographyDecisionMirrored` guard + optional `EventSnapshot` count, `lib/event-log/read/fold-snapshot.ts` exhaustiveness case, self-Ontology seed + counts 83→84 (`event-envelope.objecttype.ts` + registration test). Verified non-colliding with the reserved commit-provenance list (repo-wide grep zero hits) and deliberately non-reserved for the Path-B emit-cli mirror lane.
- **Task 1 — doc salvage**: `runtime-overlay/schemas-snapshot/ontology/INDEX.md` gains the 2026-06-15 Authority-Flow design-source pointer salvaged from the retired `~/.claude/schemas` tree, ADAPTED to the post-retirement authority direction (this package is the sole canonical contract-surface authority; the retired tree mirrored it, not the reverse); paired doc-only entry in `ontology/CHANGELOG.md`. Everything else in the home CHANGELOG was a strict already-superseded subset (verified by diff) — dropped.
- Schemas package `1.95.0 → 1.96.0` + root `CHANGELOG.md` v1.96.0 entry (Added citation satisfies `verify-event-registry-changelog` for the new type; the verifier's 45 remaining uncited types are PRE-EXISTING on main, unchanged by this wave) + `MANIFEST.json` regenerated (`refresh-schemas-snapshot-manifest --check` in sync, 249 files).

### Added
- **`scripts/emit-cli.ts`** — the P1 unification S2 cross-tree contract surface: reads exactly ONE envelope JSON object from stdin, routes it through the existing `scripts/log.ts` `emit()` (same grading via `autoGradeEnvelope`, same atomic append via `appendEventAtomic` — no reimplementation), reports `{"ok":true,"sequence":<n>}` on stdout/exit 0 or `{"ok":false,"error":"..."}` on stderr/nonzero exit. Paired test `tests/scripts/emit-cli.test.ts` (8 cases) covers a valid envelope (sequence returned, row appended, `valueGrade` stamped T1), a 5-dim-incomplete envelope (documents Path-B's existing policy: `scripts/log.ts` `emit()` has no T0 hard-reject — unlike the MCP Path-A `bridge/handlers/emit-event.ts`, which only rejects under `PALANTIR_MINI_VALUE_GRADE_ENFORCE=1` — the row is appended anyway, graded T0), and malformed JSON (rejects, nothing appended).

### Changed
- `runtime-overlay/schemas-snapshot/CANONICAL.md` — added a dated note that the `~/.claude/schemas` retirement was EXECUTED 2026-07-11 per g12 decision `de-2026-07-11-schemas-authority-ruling-plugin-self-containment-confirmed` (USER-approved), and that the `schemas-snapshot` directory name is historical (it is the source, not a snapshot of anything).

### NOT completed this wave (honest_stop — see M1 implementer report)
- **S3 value-grade module** (`autoGradeEnvelope` lift into the schemas package `ontology/lineage/`), **S2 mirrored-decision event type** (`ontology/lineage/event-types.ts`), and the **`ontology/CHANGELOG.md` + `ontology/INDEX.md` doc salvage** (task 1) were BLOCKED by this session's own live `prompt-dtc-enforcement-gate` / `ontology-engineering-workflow-enforcement-gate` governance: any Edit/Write target under a `/ontology/` path-class segment is hard-denied pending an approved `SemanticIntentContract` + `DigitalTwinChangeContract`, and that approval requires a genuine live user-turn (`promptId`/`sessionId`/`userApprovalQuote`) this dispatched executor session structurally does not have (`pm_semantic_intent_gate` resolved `sessionId: "local"`, no captured prompt-front-door envelope). The `bridge/handlers/emit-event.ts` re-import swap (task 3) is transitively blocked on the same ground (its target module lives under `ontology/lineage/`). See the M1 implementer report for the full technical grounds and recommended remediation (Lead completes the SIC/DTC turn in its own live session, or a re-dispatched executor is handed an approved `digitalTwinChangeContractRef`).
- **M1b re-attempt (2026-07-11, same session class)** independently re-confirmed the identical block from a fresh dispatch that explicitly claimed the gate was pre-opened for this work. Two sanctioned unblock paths were tried in good faith, both failing on the same root cause one governance layer earlier than the M1 attempt: `pm_semantic_intent_gate` (with a `semanticIntentContractRef` pointing at the LOCKED g12 ruling) returned `"FDE workflow provenance is required before Ontology Engineering SIC/DTC authoring, routing, or mutation"`; `pm_ontology_engineering_workflow` (`action:"start"`) returned `"pm_ontology_engineering_workflow could not resolve UniversalOntologyEntry"`. Every subpath the schemas package exports lives under `./ontology/` (its own `package.json` `exports` map has no non-ontology location), so there is no legitimate placement workaround for the S2/S3 deliverables either — this is a genuine design fork requiring a Lead-side SIC/DTC turn with live user-turn continuity, not a dispatched-executor-solvable gap. Full wiring surfaces this next attempt needs to touch (captured so it does not need to re-scout): `runtime-overlay/schemas-snapshot/ontology/lineage/event-types.ts` (`EVENT_TYPE_NAMES` + `EVENT_TYPE_REGISTRY`), `lib/event-log/types.ts` (new envelope variant + union + type guard + `EventSnapshot` count field), `lib/event-log/read/fold-snapshot.ts` (the `foldToSnapshot` switch has a compile-time `_exhaustive: never` exhaustiveness check — a new `EventEnvelope` union variant WILL fail `bun run typecheck` here until a matching `case` is added), `runtime-overlay/schemas-snapshot/ontology/self/event-envelope.objecttype.ts` (`EVENT_ENVELOPE_INSTANCES` + the "83" doc-count mentions bump to 84), and `tests/ontology/self/event-envelope-registration.test.ts` (`EXPECTED_EVENT_ENVELOPE_COUNT` 83→84). `scripts/emit-cli.test.ts`'s one remaining lint diagnostic (`'await' has no effect on the type of this expression`, bun-types under-types `.rejects.toThrow()` as `void`) is fixed this wave via an explicit `as unknown as Promise<void>` cast that preserves the real awaited assertion.

### Added — 13-site mechanical `lineageRefs` stamping (`needs-context-plumbing` census bucket)
- Purely additive `lineageRefs.actionRid` stamps on the 13 wave-3-census `needs-context-plumbing` sites (docs/2026-07-10-promotion-linkage-wave3.md §Full site table): `hooks/gates/prompt-dtc-enforcement-gate.impl.ts` (`emitGateAssessment` — `digitalTwinChangeContractRef ?? semanticIntentContractRef ?? promptId`, matching the precedence order `pm-semantic-intent-gate.ts`'s `actionRidFromLineagePayload()` already established; `promptDtcEnforcementGateImpl`'s FDE read-only skip emit — `promptId`), `lib/context/context-capsule.ts` (`archiveContextCapsule` — `capsuleId`), `hooks/task-completed-enrichment.ts` (`main` — `taskId`), `bridge/handlers/grade-semantic-intent-contract.ts` (`gradeSemanticIntentContractHandler` — `contractId`), `hooks/pre-edit-impact-mcp-first.ts` (`preEditImpactMcpFirst`, 4 sites — `optOutEnvelope.promptId`), `hooks/pre-compact-state.ts` (`preCompactState` — `capsuleId`), `hooks/subagent-start.ts` (`subagentStart`, 3 sites — `task_id`). Stamps are inert until a companion emitter shares the same rid — no behavior change to promotion outcomes from this bucket alone.

### Fixed — 3 Lead-ruled sites with companion emitters today (`scripts/replay-promote-grades.ts` ground truth: both `findT1ToT2Evidence` and `findT3ToT4Evidence` filter `payload.passed !== true` BEFORE calling `ridJoins()` — the engine's evidence predicate REQUIRES `passed === true`)
- **R-M1 — `lib/actions/commit.ts`'s F4 fail-closed refusal** (`validation_phase_completed`, `errorClass:"unregistered_action_type"`, `passed:false`) now stamps `lineageRefs.actionRid=actionTypeRid`, matching the wave-2 `commit-edits.ts` sibling checkpoints. Because the refusal is `passed:false`, the engine's passed-only gate already prevents it from ever serving as promotion evidence — no `INCIDENTAL_SIBLING_ERROR_CLASSES` change needed.
- **R-M2 — `bridge/handlers/propagation-audit-backward.ts`'s summary emit** now stamps `lineageRefs.actionRid=seedEventId` (a real `eventId`-class value, satisfying `ridJoins()`'s exact-match eventId join), stamped UNCONDITIONALLY (not gated on `firstViolationStep === null`) since the same passed-only gate already prevents a failed audit (`passed:false`) from promoting its seed.
- **R-M3 Option 1 — `edit_committed` when-ordering gap closed** (design note §(b) in the wave-3 doc): `lib/actions/commit.ts`'s `edit_committed` success envelope stamps `lineageRefs.actionRid=actionTypeRid` (part (i) of the ruling); `bridge/handlers/commit-edits.ts` emits exactly ONE new post-commit checkpoint (`validation_phase_completed`, `phase:"post_write"`, `passed:true`, `errorClass:"commit_confirmed"`) immediately after `commitEdits()` returns `committed:true`. `"commit_confirmed"` is deliberately kept OUT of `INCIDENTAL_SIBLING_ERROR_CLASSES` — unlike the 3 pre-flight siblings, it is real evidence and SHOULD count toward promotion.
  - **Fix round 1 (verified blocker):** the checkpoint originally joined to `edit_committed` via the correlation-rid join (both stamped `lineageRefs.actionRid=actionTypeRid`) — but `actionTypeRid` is a TYPE-level rid shared by EVERY invocation of a verb, so one invocation's checkpoint could promote a DIFFERENT, unrelated invocation's `edit_committed` event whenever both happened to share an `actionTypeRid` (empirically reproduced: two `edit_committed` events months apart, both promoted by one later, unrelated checkpoint). `CommitResult` now returns the per-invocation `eventId` `lib/actions/commit.ts` minted for the commit; the checkpoint stamps `lineageRefs.actionRid=<that eventId>` and joins via `ridJoins()`'s exact-match eventId join instead, making the cross-invocation promotion structurally impossible. `commit.ts`'s own `actionTypeRid` stamp on `edit_committed` (part (i)) is kept per the Lead ruling but no longer participates in this pairing.
- New regression suite `tests/scripts/replay-promote-grades-linkage-wave4.test.ts` (6 tests): (a) `edit_committed` ↔ `commit_confirmed` join promotes T1→T2 via the eventId join; (b) the 3 pre-flight siblings remain suppressed as validation evidence even when paired with `edit_committed`; (c) a stamped F4 refusal never promotes an earlier `edit_committed` sharing the same `actionTypeRid`; (d) R-M2's `passed:true` BackwardProp summary joins its seed via the eventId-class `actionRid` and promotes; (d2) a `passed:false` BackwardProp summary never promotes its seed (pass-only guard); (e) fix round 1 regression: a `commit_confirmed` checkpoint never promotes a different, unrelated invocation's `edit_committed` sharing the same `actionTypeRid`.
...
### Changed — PreToolUse `*`-matcher gates become thin fast-path entries (H3)
- **`hooks/ontology-engineering-workflow-enforcement-gate.ts` + `hooks/prompt-dtc-enforcement-gate.ts` are now thin CLI entries**; the full gate logic moved byte-identically to `hooks/gates/{ontology-engineering-workflow-enforcement-gate,prompt-dtc-enforcement-gate}.impl.ts` (new `runFromPayload` export; shared `LEGACY_RUNTIME_UI_TOKENS`/`ONTOLOGY_ENGINEERING_MARKERS` arrays extracted to `hooks/gates/pretool-fast-path-constants.ts`). Both gates previously loaded their full module graphs on EVERY tool call (matcher `*`, 2 spawns/call).
- **Fast path** (skip verdict printed without loading the heavy graph, ~2× faster per spawn): tool name ∈ `CLAUDE_TOOL_PROFILE.readOnlyTools` (read/grep/glob/ls/notebookread — the classifier's canonical set, imported not duplicated), AND (gate 1) no OE marker / legacy-UI token anywhere in the payload, AND (gate 2) no explicit `PALANTIR_MINI_PROMPT_DTC_GATE_MODE`, `PALANTIR_MINI_PROMPT_DTC_GATE_BYPASS` ≠ 1, project root resolves non-excluded, and no current FDE ontology-engineering session. Everything else — mutations, Bash, MCP tools, unknown/future tools, marker-bearing reads, env overrides, active FDE sessions, out-of-project cwd — takes the full path unchanged; fail-closed semantics preserved (gate-2 unexpected errors still deny).
- **Governance surfaces untouched**: `hooks/hooks.json` commands/matchers unchanged, ontology self-model hook seed unchanged (hooks/ root stays 49 files — `hooks/gates/` is invisible to the non-recursive count guard), `palantir-mini/hooks/` protected-surface marker still covers the moved impls.
- 13-case differential test `tests/hooks/pretool-gate-fast-entry.test.ts` pins entry-vs-impl byte equality across fast-path and fall-through lanes; 10 existing test files repoint gate imports to the impl paths (`tests/integration/dtc-end-to-end-smoke.test.ts` source-text assertions included).

### Added — sync-codex-adapter CI coverage (H7)
- **`tests/scripts/sync-codex-adapter.test.ts` rewritten** (generator script itself unchanged): CI-safe invariants — `--dry-run` exits 0 and writes nothing, event allowlist == sorted `hooks.json` keys (header list + count), thin-shim contract (AUTO-GENERATED header, exactly one import, `runCodexHookAdapterCli()` delegation), byte-identical determinism across runs (mtime-derived `generatedAt`), invalid-hooks.json failure lane on a minimal fixture tree — plus an existence-gated deployed-shim drift check (byte-compares `~/.codex/hooks/palantir-mini-codex-hook-adapter.ts` against a fresh dry-run on the author machine; skips in CI). All pre-existing assertions retained (shebang, plan-v2 6.2 ref, retired-path negative checks, adapter-boundary sentence, precondition existence checks). The manual-only generator is now exercised on every CI `bun test` run.

## [7.38.0] - 2026-07-02 — H8 doc-drift cleanup + pm-rule retirement, repo-wide path portability (H6), full test suite in CI (H4)

Session-2 waves A+B. **Wave A (docs/surface)** removes every verified-false claim from the living docs (a nonexistent auto-fire hook, seven "legacy hooks" whose files don't exist), documents the SecondBrain lane in README + SSOT-AUTHORITY for the first time, retires the self-flagged `pm-rule` delete-candidate skill (zero live consumers by repo-wide grep), and prunes the stale broad-suite failure ledger. **Wave B (portability + CI)** removes author-machine `/home/palantirkc` hardcoding from every load-bearing surface (hooks, bridge handlers, lib consts, dev scripts, skills, authority declarations, schemas-snapshot seeds), converts SSoT authority declarations to a machine-neutral `~` form, fixes a governance classifier gap and a silently-no-op test mock found along the way, and — with the suite finally green on any machine — adds `bun test` (3,750+ tests) to CI, which previously ran only typecheck + two integrity scripts. schemas-snapshot moves 1.87.0 → 1.95.0 across the arc (1.94.0 pm-rule seed retirement; 1.95.0 portability; the 1.88–1.93 gap is pre-existing CHANGELOG-only history whose package.json bump was never applied — now reconciled). A post-CI **wave C (fix)** resolves the failures the new CI suite itself exposed — inverting the memory-reflect→pm-recap dependency behind an injected `RecapProvider` (ADR 0001) and making the prune-retention fixture deterministic.

### Removed — pm-rule skill (delete-candidate retirement, schemas 1.94.0)
- **`skills/pm-rule/` deleted** — self-flagged `category: delete-candidate` / `surfaceStatus: deprecated-candidate`; repo-wide consumer grep found zero live consumers. Hint strings in `hooks/value-grade-assigner.ts` + `scripts/rule-excerpt.ts` drop the slash-skill suffix and keep the `pm_rule_query` MCP guidance; dangling prose refs pruned from `skills/pm-rule-audit/SKILL.md` + `runtime-overlay/rules/AUTHORING.md`. Seed rows removed from BOTH seed surfaces (`ontology/seeds/skill-definitions.ts` AND `ontology/self/skill.objecttype.ts` — the second was caught by the independent verifier), `registeredSkills` 45 → 44 in `plugin-manifest.objecttype.ts`, count pins updated. Skill count 45 → 44.

### Fixed — doc drift (H8)
- **`skills/pm-cold-start-orchestrate/SKILL.md`** no longer claims the nonexistent `cold-start-browse-index-loader` SessionStart auto-fire hook (removed as an orphan in 6.132.0) and no longer hardcodes `/home/palantirkc/.claude/research/**` (now `~/.claude/research/**`); the schemas seed description was aligned in the same pass.
- **README "Legacy hooks" paragraph rewritten** — all seven named hooks verified absent from disk and from `hooks/hooks.json`/`codex-hooks.json`; the paragraph now points at the actually-wired `prompt-front-door-capture` instead.
- **`tests/KNOWN_BROAD_SUITE_FAILURES.md` pruned 29 → 0 rows** — 4 rows referenced deleted test files, the rest re-verified passing; the final flaky row (wave2-mvp AST evidence) resolved by raising that single test's timeout 10s → 30s. Ledger now empty.
- **`tests/docs/reload-per-runtime.test.ts`** updated to the doc's current Codex+Claude multi-runtime reality (test lagged the deliberate 2026-06-15 doc rewrite).

### Added — SecondBrain documentation
- **README `## SecondBrain Fold` section + SSOT-AUTHORITY data-layer authority facts (v1.9.0)** — Stop-hook bookmark → `manifest.json.foldedSessions` sole fold-lifecycle authority → SessionStart dispatch → schema-governed NDJSON contract → T-tier retention; `graph.json` content documented as engine/consumer-owned, events.jsonl remains the lineage spine. Neither doc mentioned SecondBrain before.

### Fixed — path portability (H6, schemas 1.95.0)
- **`process.env.HOME ?? "/home/palantirkc"` → `?? os.homedir()`** across 17 hooks/bridge files (both live PreToolUse gates included — fallback-only expressions, semantics unchanged when HOME is set), 6 schemas-snapshot files, 7 dev scripts.
- **No-fallback hardcoded consts resolved via home/env**: `lib/consumer-peerdep-scanner.ts` (new `PALANTIR_MINI_CONSUMER_PROJECTS_ROOT` override; name avoids the existing, differently-scoped `PALANTIR_MINI_PROJECTS_ROOT`), `lib/codex/palantir-mini-activation-policy.ts`, `lib/codex/codex-hook-adapter.ts`, `lib/evidence/evidence-source-policy.ts`.
- **Authority declarations machine-neutral (`~` form)**: `.ssot-authority.json` v1.8.0, `contracts/layer-boundary.contract.json` (7 refs), `ci/verify-marketplace-integrity.ts`; the out-of-repo runtime-boundary contract stays out-of-repo by design — its content tests are now existence-gated (run on the author machine, skip elsewhere), and the source-root sentinel's legacy-marker check gains a negative lookahead so the new canonical value is not a false positive.
- **`scripts/bootstrap-home.sh` genuine bug**: the Claude project-slug segment (`-home-palantirkc`) was hardcoded — off the author machine it silently checked the WRONG memory dir while reporting OK; the slug is now derived from `$HOME` (`tr '/' '-'`, matching the JS `path.resolve().split(sep).join("-")` encoding).
- **`lib/chatbot-studio/data-core.ts`** citation refs + the line-750 runtime gate canonicalized to the `~/.claude/research/...` string form (+ test and eval fixture); redundant absolute prefix dropped from `palantir-source-authority` defaults.
- **Composers fall back to plugin-bundled overlays** (`lib/ontology-context/{retrieval-context,application-state}.ts`): rules and schema-primitives sub-fields fall back to `runtime-overlay/rules/` / `schemas-snapshot` primitives ONLY when the external `~/.claude` dirs are absent — external-preferred polarity keeps author-machine behavior byte-identical; aligning composers to resolve-rule's plugin-overlay-first policy is a deferred design question.
- **Test-only portability fixes**: fresh-home restore gating, memory-reflect `mock.module` specifier derived at runtime (the hardcoded absolute path meant the mock NEVER intercepted — the long-suspected memory-reflect "mock bug"), lead-ontology-discovery E1 fixture, config/root cwd fixture, semantic-drift-audit palantir-math smoke existence-gated, impact-query/response-template fixtures.

### Added — CI runs the full test suite (H4) + classifier hardening
- **`bun test` step added to `palantir-mini-integrity.yml`** — CI previously ran only typecheck + 2 integrity scripts + cartography drift; it now runs the full suite (3,756 pass / 0 fail / 9 skip on a clean environment; machine-specific integration tests skip via existence gates).
- **`lib/hooks/tool-classifier.ts` narrow fail-closed denylist** — script-interpreter invocations (`bun run`/`bunx`/`bun`/`node`/`npx`/`tsx`) of mutation-named scripts (`apply-`/`migrate-`/`sync-`/`commit-`/`write-`/`mutate-`/`delete-`/`remove-`/`register-` prefixes, `*governed-change*`) are no longer classified read-only. Fixes the activation-policy protected-tool test that had never passed since it was authored. Anchored regex + basename-only matching avoids the bd-003 over-block class (15-command read-only battery green, incl. name-collision traps). `python`/`sh` interpreter coverage is a pre-existing gap, backlogged.

### Fixed — memory-reflect/pm-recap dependency inversion + prune test flake (post-CI wave C, ADR 0001)
- **`lib/runtime-overlay/memory-reflect.ts` no longer imports `bridge/`** — the dynamic `await import("../../bridge/handlers/pm-recap")` (a lib→bridge edge unlisted in the H1 ledger) is replaced by an injected `RecapProvider` capability (`lib/recap/types.ts`); `hooks/t4-promotion-trigger.ts` wires in the real `pm_recap` handler, unwired callers degrade to the existing minimal-digest fallback (`docs/adr/0001-memory-reflect-recap-provider-inversion.md` — the repo's first ADR).
- **Fixes the PR #206 CI failures in `pm-recap.mcp-first-compliance.test.ts`** — Bun's `mock.module()` is process-global and file-load-order dependent; once wave B fixed the silently-no-op mock specifier, `tests/lib/runtime-overlay/memory-reflect.test.ts` began intercepting the pm-recap import of any test file loaded after it (CI's load order differed from the local alphabetical one). `mock.module()` is removed entirely from the test in favor of directly-injected fake providers — the pollution class is gone by construction.
- **`tests/hooks/pre-compact-state.test.ts` prune-fixture flake fixed** — the "ignores non-snapshot files" test raced fractional-ms `mtimeMs` against integer-ms `Date.now()` at `maxAgeMs: 0` (110/500 reproduction); the fixture now prunes 24h-aged snapshots against a 1h `maxAgeMs`, deterministic by a ~23h margin. Production keep-policy untouched (boundary characteristic documented in `lib/event-log/snapshot.ts`).
- **pm-recap result types promoted to `lib/recap/types.ts`** (`PmRecapArgs`, `PmRecapResult`, `SubstrateHealth`, `SprintSummary`) — previously file-local to the bridge handler; `ClassificationAccuracySummary` gains a proper export in `lib/recap/classification-accuracy.ts`. Handler behavior byte-identical.
...
### Hardened — DTC gate: A2 authorized-delivery lane + ontology-write-bypass closure (SECURITY)
- **A2 authorized-delivery lane** — `assessPromptDtc` gains a delivery early-return that PASSES-WITH-AUDIT when a re-verified, on-turn, unforgeable user-approval envelope authorizes a delivery (merge / PR / commit / release / push) action. Routed through the new `verifyDeliveryApprovalAgainstEnvelope` / `excerptExpressesDeliveryApproval` (the PreToolUse mirror of the DTC-build verifier; FAIL-CLOSED; bounded approval-verb↔delivery-marker co-occurrence window). Returns `undefined` (legacy byte-identical) when no approval inputs are supplied (`hooks/prompt-dtc-enforcement-gate.ts`, `lib/lead-intent/dtc-build-approval.ts`).
- **Ontology-write-bypass closed** — eligibility for the A2 PASS is NOT class membership: the new `isProvenNonOntologyDelivery` guard re-proves the operation is non-ontology by BOTH the tool classifier AND the resolved write-set before any pass, so an ontology write that de-floors to `commit`/`generic-mutation` can no longer slip the delivery lane. `AUTHORIZED_DELIVERY_CLASSES` positively enumerates the eligible delivery surfaces and DELIBERATELY excludes `ontology-write`; `effective-gate-mode`'s blunt env `off` stays strengthen-only (floor-neutral `hasExplicitGateMode` plumbed for audit only) (`lib/governance/effective-gate-mode.ts`, `hooks/prompt-dtc-enforcement-gate.ts`).
- **Path-predicate unification** — the A2 write-set screen, the staged/push-range de-floors, and the scoped-blocking advisory now all fold in the canonical case-insensitive `pathIsProjectOntologyClass` (object/link/action/interface-type, shared-property, any-case `ontology/`), closing the bypass where ontology-class siblings / case-variants leaked through as "non-ontology" → A2-eligible (`hooks/prompt-dtc-enforcement-gate.ts`).
- **bd-019 — cross-repo `git` honored** — the staged-set de-floor now honors `git -C <repo>` / `--work-tree[=]<dir>` (new `isGitSubcommand` tolerates git's global options between `git` and the subcommand; `resolveGitTargetRepo` resolves the actually-mutated repo), so a cross-repo `git -C <other-repo> commit` is evaluated against the repo it mutates instead of the session CWD (`hooks/prompt-dtc-enforcement-gate.ts`).

### Closed — BackwardProp Sink-1 (READ + prior-digest reflect)
- **NEW `a2-prior` READ** — at SessionStart, surfaces ONE compact `[A2-prior]` `additionalContext` line folding the most recent valuable decisions (already-promoted T3+, fold-verdict BY-TYPE incl. a fresh T2 `lead_decision`, and outcome-paired rows). Read-only + tail-only (last ~2000 lines) + best-effort; pushed UNCONDITIONALLY each SessionStart so a fresh T2 surfaces NEXT session BY TYPE before promotion grades it (`lib/runtime-overlay/a2-prior.ts`, wired in `hooks/session-start.ts`).
- **`memory-reflect` repointed to a pm-owned cache** — the session-end prior-digest reflection now writes a DEDICATED `.palantir-mini/cache/memory-prior.md` fenced cache (auto-created) and NEVER writes the curated git-tracked MEMORY.md (index-only, USER decision). New `PALANTIR_MINI_MEMORY_REFLECT_DISABLE=1` bypass (`lib/runtime-overlay/memory-reflect.ts`).

### Closed — BackwardProp Sink-2 (grade-promotion replay)
- **`t4-promotion-trigger` rewritten to an offline grade-promotion replay** — at session end (Stop, advisory/async, non-blocking) it replays T1→T2→T3 promotions over the project's `events.jsonl`, appending NEW `promotedFrom` rows (append-only + idempotent: a 2nd consecutive run promotes 0, no duplicates) so graded value does not silently expire. SOLO-DEV CEILING: drives T1→T2→T3 only; never calls `promoteT3ToT4` / injects `kLlmConsensus` (T4 stays gated-but-correct). New `PALANTIR_MINI_PROMOTE_DISABLE=1` bypass (`hooks/t4-promotion-trigger.ts`, batch CLI `scripts/replay-promote-grades.ts`).

### Reconciled — P2 / S0: rule-26 doc↔code grading-taxonomy drift (v2.1.0)
- **rule 26 body 2.0.0 → 2.1.0** — transcribed the LIVE `autoGradeEnvelope` T0–T4 + D2 + 14-criteria taxonomy into the body and refuted the v2.0.0 "dropped T0–T4 detailed scoring / multi-vendor consensus" claim that was **never adopted in code**. Documents the T4/D2 multi-vendor ceiling as STRUCTURALLY UNREACHABLE under the permanent single-vendor (Claude-only) deployment with `promoteT3ToT4` left gated-but-correct (`runtime-overlay/rules/26-valuable-data-standard.md`).
- **NEW drift-guard conformance test** — pins the body's tier→axis mapping (T1=A+E, T2=+B, T3=+C, T4=+D2) to the actual `autoGradeEnvelope` branch order and asserts the stale "dropped" claim never returns (`tests/governance/rule-26-doc-grading-conformance.test.ts`).

### Added — P3 Lead-decision governed-emit
- **NEW `lead_decision` event type** — a Lead orchestration decision recorded as a governed Layer-1 audit event. Registered in the event-type registry (`runtime-overlay/schemas-snapshot/ontology/lineage/event-types.ts`) + the `EventEnvelope` discriminated union, `EventSnapshot` counter, fold-snapshot reducer case, and `isLeadDecision` type guard (`lib/event-log/types.ts`, `lib/event-log/read/fold-snapshot.ts`). The `a2-prior` fold-verdict BY-TYPE branch already whitelists `lead_decision`, so a freshly-emitted T2/T3 `lead_decision` surfaces NEXT session by TYPE before any promotion grades it.
- **NEW Path-B emit CLI** routes a Lead decision through the in-process `emit()` (Path B) instead of the altitude-2-hidden MCP `emit_event` tool — maps `decisionJson.{reasoning, hypothesis?, refinementTarget?, memoryLayers, lineageRefs?}` onto the 5-dim `withWhat` so the in-band auto-grade reaches ≥T2 (T3 with `refinementTarget`); identity = the REAL runtime (never `monitor`); appends-only so its `bun`-run invocation classifies read-only/allowed (`lib/lead-intent/lead-decision-emit-cli.ts`, paired/dogfood test `tests/lib/lead-intent/lead-decision-emit-cli.test.ts`).
- **DOGFOOD proven** — the paired test emits a T3 `lead_decision` (with `refinementTarget`) into a fixture `events.jsonl`, runs `a2-prior` against it, and asserts the decision string folds back into the returned `[A2-prior]` line.
...
### Fixed
- fix(tool-classifier): `isReadOnlyBashCommand` was allowlist-wins / default-DENY — its final positive `^(pwd|ls|find|...)` first-token allowlist classified every unknown leading verb (`du`, `jq`, `awk`, `sort`, `ps`, `stat`, `echo`, `column`, `comm`, …) as a MUTATION, so benign inspection Bash was logged/assessed as a protected mutation and lost the off/env bypass early-exits. Inverted to **denylist-wins / default-ALLOW**: the strengthened anchorless write denylist (redirect incl. `>|` clobber + bare-trailing, `tee`, sed/perl `-i`, fs verbs incl. `rmdir/ln/install/dd/truncate/mkfifo/mknod/shred`, pkg-mgr install/add/remove/update incl. short `i/un/rm`, pip, git-write incl. `restore/fetch/tag/…`, gh-PR-write, deploy) is now the sole "does-this-write" signal; the first-token allowlist is removed. **Decide on the RESOLVED write structure, not first-token vocabulary.**
- fix(gate): `ontology-engineering-workflow-enforcement-gate` armed BOTH the FDE-provenance deny and the protected-surface-mutation deny off `collectPathLikeValues`, whose key regex (`…|command|prompt|intent|content`) harvested FREE-TEXT adapter fields — so a memory-NOTE `Write` whose `content` merely *described* an ontology re-bind (mentioning `palantir-mini/lib/ontology-engineering-workflow/` or a `/ontology/` segment) was denied even though the resolved write target was a plain `.md` (2026-06-21 recurrence, session `9c5344e3`). Replaced with `resolveWriteTargetPaths`/`resolveWriteTargetAbs` (host write-tool target keys ONLY: `file_path`, `notebook_path`, `edits[].file_path`; abs+normalized so relative/`~` forms still match), routed through `targetsProtectedSurface`, `targetsProjectOntologyPathClass`, and the source-mutation fast-path `touchedPaths`; `collectPathLikeValues` + its key regex DELETED. One swap fixes BOTH branches (shared `protectedSurfaceMutation` flag).
- fix(gate): + resolved-Bash-write-target extraction closes the Bash protected-surface vector. The `collectPathLikeValues` removal above also dropped the (over-blocking) free-text `command` scan, which silently un-guarded a MUTATING Bash that writes to a protected pm-self surface (`echo x > …/palantir-mini/hooks/x.ts`, `sed -i … skills/…`, `tee …/lib/lead-intent/x.ts`, `mv`/`cp`/`dd into …/hooks/`): such a command resolved to an EMPTY write-target set ⇒ `targetsProtectedSurface=false` ⇒ the provenance + mutationAuthorized deny was dropped. `resolveWriteTargetPaths` now parses the RESOLVED write-target token(s) from a Bash `command` (redirect dest incl. `>|`/`&>`/`1>`/`2>`, `tee [-a] FILE…`, sed/perl `-i FILE`, `mv`/`cp`/`install DEST`, `dd of=FILE`, `truncate`/`touch`/`ln FILE`) and resolves each abs via `resolveWriteTargetAbs` — the structured write TARGET, never the command vocabulary. A read-only Bash (no write operator) yields no token ⇒ empty set ⇒ the over-block relaxation is PRESERVED (`grep`/`cat` mentioning a protected path still PASS); a write whose target is out-of-tree (`echo "…palantir-mini/hooks" > /tmp/note`) resolves to the out-of-tree path and PASSES. Out of scope (known cluster-A follow-up): tool-classifier mutation-detection gaps (sponge, python/node writer scripts, `curl -o`, `wget`, `gh api POST`, `tar xf`).

### Changed
- `lib/ontology-engineering-response-template.ts`: the mandatory governed footer is now keyed to LIVE governance **STATE**, not prompt/response VOCABULARY. ADD `PALANTIR_MINI_GOVERNED_ACTIVE_STATES` (the 8 non-pole front-door states), `isPalantirMiniWorkflowResponseRequiredForState(state)` (footer required iff a governed SIC/DTC contract is OPEN and not opted out; opt-out read from the authoritative `envelope.palantirMiniPluginOptOut`), and the `promptTextSuggestsPalantirMiniWorkflow(text)` recall hint. `isPalantirMiniWorkflowResponseRequired` is now an `@deprecated` thin alias over the recall hint (back-compat preserved). Call sites updated: `prompt-front-door-capture` (STATE from the in-hand envelope — raw-capture `captured` turn no longer auto-injects the footer), `stop-validate` (reads `session_id` → resolves the current envelope via `PromptFrontDoorStore`, STATE-first with recall-hint fallback when no envelope/session is resolvable), `pm-workflow-response-validate` (dormant/removed tool → renamed to the recall hint).

### Notes
- **relaxes over-block**: benign inspection Bash is now read-only; a non-governed turn (`captured`/`superseded`/no-envelope) that merely mentions ontology vocabulary no longer requires the ~3.2k footer; a memory-note/out-of-tree `.md` Write whose content describes ontology work now PASSES the enforcement gate.
- **under-block-safe = genuine protected mutation still blocks**: real mutating Bash (redirect/tee/sed -i/git-write/rm/dd/pkg-install) still classifies NOT read-only ⇒ `isProtectedMutation=true`; a real write to a protected pm-source surface (`file_path`/`notebook_path`/`edits[].file_path` resolving under `palantir-mini/hooks|lib|skills/…`, the installed-cache form, a relative/`~` target, or a project `/object-type/` path-class with no OE marker) still hits the provenance + mutation denies; a genuinely governed-active turn (any of the 8 states incl. `digital_twin_approved`) still gets the footer; the Stop recall-hint fallback only ADDS coverage when state is unavailable.
- **decide-on-resolved-target-not-vocabulary**: the unifying principle across all three sub-fixes — no gate bypass, no guard disabled; the ARMING SIGNAL (resolved write target / live STATE) is corrected, the deny branches are unchanged. Generalizes bd-002 (footer) + bd-003 (read-only Bash + free-text harvest).
- Tests: tool-classifier RO/MUT both directions + classify chain; STATE predicate full lifecycle + non-pole relaxation + opt-out-wins + recall-hint; gate relaxation (memory-note/out-of-tree/read-only-Bash/OE-marker-content PASS) + under-block (protected pm-source Write/relative Edit/MultiEdit/NotebookEdit/`/object-type/` path-class still BLOCK); stop-validate STATE-first governed=fail / captured=pass / no-envelope-fallback=fail. NEW-fail-0, NEW-tsc-errors-0.

Files touched: `lib/hooks/tool-classifier.ts`, `lib/ontology-engineering-response-template.ts`, `hooks/prompt-front-door-capture.ts`, `hooks/stop-validate.ts`, `bridge/handlers/pm-workflow-response-validate.ts`, `hooks/ontology-engineering-workflow-enforcement-gate.ts`, plus their tests, plus version bump (`package.json`, `.codex-plugin/plugin.json`, `.claude-plugin/plugin.json`).
...
### Added
- feat(gate): a STRUCTURAL "0-new-term DYNAMIC re-bind" lane in the ontology-DTC promotion / build-readiness gate. New exported predicate `isZeroNewTermRebind(input)` in `lib/lead-intent/contracts.ts` proves a change introduces ZERO new ontology rids: it derives the affected-rid set from the DTC's own `touchedOntologyRefs` and requires EVERY affected rid to already appear in the caller-supplied `registeredOntologyRids` (the rids the fs-bound caller folds out of `snapshot.registeredPrimitives`, mirroring `elevate.ts`'s register seam). FAIL-CLOSED: absent registry, empty/absent `touchedOntologyRefs`, or a single touched rid not in the registry ⇒ FALSE ⇒ the FULL construction gate applies. The relaxation is therefore UNREACHABLE for any change that adds a rid. Mirrors the existing `userApprovalAuthorizesDispatch` (Improvement #3) precedent: a pre-verified signal; the pure lib never reads fs.
- feat(gate): in `assessOntologyDtcBuildReadinessGate`, the structural rebind result (`const isRebind = isZeroNewTermRebind(input)`, computed once) is folded into the EXISTING `satisfiedByEnvelope` / `envelopeSatisfied` substitution — extending it (no parallel mechanism) so a verified rebind satisfies ONLY the construction-only checks: `workContractRef`/`routerBindingRef` ref-presence, WorkContract/RouterBinding body-dereference, WorkContract/RouterBinding validity, and the gate-level eval / branch-policy / permission-policy build-evidence checks. UNCHANGED + mandatory for ALL changes: SIC/DTC ref-present, SIC/DTC body-dereferenced, SIC/DTC body-validated (incl. the DTC body's own `requiredEvaluationRefs` requirement), and `approval-ref-present`.
- feat(gate): additive `isZeroNewTermRebind?: boolean` on `SemanticConsistencyPromotionGateInput` (`lib/semantic-consistency/promotion-gate.ts`). When the pre-verified signal is true, semantic consistency is vacuously satisfied (a 0-new-term change has no term to resolve): the three resolver-evidence blocking findings `SEMANTIC_CONSISTENCY_RESULT_MISSING`, `SEMANTIC_CONSISTENCY_EMPTY_RESULT`, and `SEMANTIC_CONSISTENCY_RESOLVER_RUN_REF_MISMATCH` are skipped. NON-deterministic / LLM-promotion-used / unresolved-blocking-conflict findings are NEVER relaxed.
- feat(wiring): the lane is WIRED into the production register/route path (the inputs are no longer test-only / dormant). New PURE, fail-closed `lib/lead-intent/registered-ontology-rids.ts` `deriveRegisteredOntologyRids(projectRoot)` reads ONLY the genuine governed snapshot (`getOntology(project).snapshot.registeredPrimitives`, the same fold `elevate.ts`'s register seam reads) and flattens it to bare rids across all six buckets; a missing project / read failure / absent snapshot returns `undefined` (NEVER a partial/guessed list ⇒ legacy). Both production gate handlers now derive it ONCE per invocation and thread it in: `bridge/handlers/pm-semantic-intent-gate.ts` (into `assessContractGate` + `assessSemanticGateOntologyDtcBuildReadiness`) and `bridge/handlers/pm-intent-router.ts` (into `assessContractGate` + `projectRoutingFromContracts` + both `assessRouterOntologyDtcBuildReadiness` calls). The semantic-consistency relaxation is driven by `const isRebind = isZeroNewTermRebind(...)` DERIVED inside `semanticConsistencyPromotionIssuesForContracts` (contracts.ts) from the SAME threaded `registeredOntologyRids` — never an independent/free boolean. `registeredOntologyRids?` added to `LeadIntentGateInput`.

### Notes
- ADDITIVE ONLY: every non-rebind path (no `registeredOntologyRids` / `isZeroNewTermRebind` absent or false) is byte-identical legacy behavior — verified by the unchanged legacy suites plus the 7 pre-existing (unrelated) repo failures being byte-identical before and after (same test names). No universal invariant relaxed (SIC/DTC validity, approval-ref, approver≠author, ActionType=sole-commit-gate, 5-dim lineage emission all untouched); only construction-artifact requirements meaningless for a 0-term re-bind are relaxed, gated on the SAME fail-closed structural predicate sourced ONLY from the live snapshot.
- New tests prove the truth table AND the production wiring: pure re-bind (all touched rids registered, zero new) ⇒ predicate true ⇒ construction checks relaxed ⇒ readiness reachable + semantic-consistency findings skipped; ANY new rid / missing structural proof / empty touched-refs / draft body / unavailable snapshot ⇒ predicate FALSE ⇒ full construction gate UNCHANGED. A real register-shaped `events.jsonl` snapshot is folded end-to-end to confirm the lane activates via the live derivation.

Files touched: `lib/lead-intent/contracts.ts`, `lib/lead-intent/registered-ontology-rids.ts` (new), `lib/semantic-consistency/promotion-gate.ts`, `bridge/handlers/pm-semantic-intent-gate.ts`, `bridge/handlers/pm-intent-router.ts`, `tests/lib/lead-intent/contracts.test.ts`, `tests/lib/lead-intent/registered-ontology-rids.test.ts` (new), `tests/lib/semantic-consistency/promotion-gate.test.ts`, plus version bump (`package.json`, `.codex-plugin/plugin.json`, `.claude-plugin/plugin.json`).
...
### Added
- Gate front-door mutationMode lane (Slice B — the SECOND `mutationAuthorized` producer; PR #155 / 7.8.0 shipped the first, the OE-workflow state). `bridge/handlers/pm-semantic-intent-gate.ts`'s `buildWorkflowContractProjection` now fans its single `mutationAuthorized` boolean across the seven SSoT modes via `deriveMutationAuthorizationByMode`, surfacing `mutationMode` + `mutationAuthorization` on the `SemanticIntentWorkflowContractProjection`. PURELY ADDITIVE: the legacy `mutationAuthorized` boolean stays bound to the promotion lane and is never widened. A declared `consumer-data-write` (with `consumerActionTypeRef` + `consumerWriteValidated`) routes via `allowedNextActions: ["route-with-mutation-mode:consumer-data-write", "do-not-route-via-promotion-gate", …]` — a consuming-layer write is no longer forced through the 9-axis SIC+DTC promotion gate. (Handler edits authored in a separate lane; this entry documents the round.)
- P3 decision round-trip advisory: a `turn` on `pm_ontology_engineering_workflow` that minted `turnDecisionSpecs` with no matching `userDecisionRecord` now returns `decisionAdvisory` echoing the still-open `choiceId`s (with `kind` + `label`) and NAMING the input field — `choiceApplications` — that records them, so the `decision-ledger.forward-only-existing-gap` warning is actionable without reverse-engineering the input schema. Drops to absent once every spec round-trips.
- P5b phase/next-action self-location: `turn` responses now carry `phaseAdvisory` (the current `phase` + the canonical next action = head of the derived `allowedNextActions`), and the gate projection states `currentPhase` + a canonical head action — a tool response self-locates instead of leaving the next call to inference.
- P6 one-hop, auditable FDE-provenance front door: when an Ontology-Engineering intent reaches the gate WITHOUT FDEOntologyEngineeringSession provenance, the projection now names the EXACT next call (`pm_ontology_engineering_workflow start`) as the head of `allowedNextActions`, AND the bounce is persisted as its OWN 5-dim event (`validation_phase_completed`, distinct `errorClass: "fde_provenance_required"`, `requiredNextAction: "pm_ontology_engineering_workflow start"`, `refinementTarget` → `bridge/handlers/pm-semantic-intent-gate.ts`) — closing diagnosis P6 part B (the prerequisite previously surfaced only as an in-transcript error and was never written to events.jsonl).

### Changed
- Persisted-state schema version UNCHANGED at `palantir-mini/ontology-engineering-workflow/v2`: this round added fields only to in-memory RESPONSE/projection surfaces (`decisionAdvisory`/`phaseAdvisory` on the handler result; `mutationMode`/`mutationAuthorization` on the gate projection) and READS off existing state (`state.phase`, `state.allowedNextActions`, `state.turnDecisionSpecs`, `state.userDecisionRecords`); `OntologyEngineeringWorkflowState` and its writer (`store.ts`) were not touched, so the persisted SHAPE did not change. The ssot/ contract lift for these behaviors landed separately in `harness-upstream`.

Verification: `bun run typecheck` exit 0; full suite 0 fail (3267 pass / 16 skip) — adds 13 paired behavioral tests across 2 files: P3 decisionAdvisory echo + choiceApplications round-trip clears it (`tests/bridge/handlers/pm-ontology-engineering-workflow-decision-phase-advisory.test.ts`), P5b phaseAdvisory phase+next-action, the Slice B ROUTING-INVARIANT LOCK (default/explicit-default mutationMode ⇒ identical `mutationAuthorized` + `currentPhase` + `allowedNextActions`, anchored to the literal pre-change action list; consumer-data-write lane authorizes WITHOUT the promotion gate), and P6 FDE-provenance one-hop + the 5-dim event payload shape (`tests/bridge/handlers/pm-semantic-intent-gate-mutation-mode-routing.test.ts`). Three-manifest version-lane bump (package.json + .claude-plugin/plugin.json + .codex-plugin/plugin.json) 7.8.0 → 7.9.0 stays green (dtc-end-to-end-smoke equality test).
...
### Added
- **`bridge/handlers/grade-outcome-with-rubric.ts` + `lib/grader/grade-outcome.ts`** — the runtime-neutral `grade_outcome_with_rubric` MCP tool (referenced everywhere but never built/removed in Wave 2D/2E). Thin handler over a PURE core: deterministic domains (`rule` regex/minimal-JSONSchema, `hybrid` combinator, `human` review-flag) scored in-process; effectful domains are INJECTED — `code` runs `validationExpression` as a shell command, `model` delegates to `pm_grader_dispatch`. Emits `evaluator_strictness_probe` per model criterion + `grading_completed`. Accepts inline `rubric` or a registered `rubricId` (GRADING_RUBRIC_REGISTRY). Aggregates weighted normalized scores; `human` criteria counted separately. Domains `simulator`/`visual` are skipped (no neutral backend — survey-corrected: the RubricDomain union is 7-value but only 5 have backing).
- **`bridge/handlers/pm-grader-dispatch.ts` + `lib/grader/dispatch-adapter.ts`** — the runtime-neutral `pm_grader_dispatch` MCP tool. Dispatches ONE model-domain criterion to a FRESH grader subprocess (eliminates self-grading bias, rule 16 §Roles). The CONTRACT (effort routing via `grader-effort` helpers, runtime selection, last-JSON-line parse, graceful degrade) is neutral; the spawn (`claude -p --effort` / `codex exec`) is the ADAPTER binding selected via `PALANTIR_MINI_HOST_RUNTIME`. Never throws — binary-unavailable / timeout / malformed output degrade to score=0 (preserves the modelGrader-absent default-0 invariant). Spawn idiom + ENOENT/EACCES/timeout handling mirror `pm-preamble/effort-probe.ts`; injectable `execFn` so CI never spawns a real binary.
- Both tools registered across all 3 load-gates (`TOOLS` + `TOOL_CATEGORIES.harness-engineering` + `HANDLER_MODULES` in `bridge/mcp-server.ts`; capability metadata in `lib/capability-registry/mcp-tool-capability.ts` — `grade_outcome_with_rubric` externalEgress:false, `pm_grader_dispatch` externalEgress:true) + the `managed-settings.d/50-palantir-mini.json` RBAC allowlist (rule 07). Unblocks the dangling consumers `skills/pm-eval-suite-run`, `agents/lead-orchestrator`, `agents/eval-judge`.

**0 new regressions** (env-clean stash-baseline-diff IDENTICAL: 8 pre-existing env/checkout-specific fails before and after; the apparent 25-vs-20 delta on the dev host was the stale `PALANTIR_MINI_PLUGIN_ROOT=…/cache/…/6.84.1` reading a 27-tool cached plugin against the live 29-tool surface). 24 new unit tests (pure-core domains, hybrid combinators, dispatch effort/runtime routing + graceful degrade, handler validation, registration load-gate smoke). `pm_plugin_self_check` public-mcp count 27→29 updated. No schema primitive touched; M-SELF counter unchanged (this wave registers MCP handlers, not self-Ontology instances — Step 6.5 lands in W3e-3 with the Executor ActionType).
