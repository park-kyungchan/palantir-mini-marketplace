...
| **Hook added/modified** (hooks/\*.ts, hooks/hooks.json) | **Yes** | Hooks are registered from hooks.json at session start; in-flight sessions retain the prior hook set |
...
- If `hooks/hooks.json` is modified (hook registration table), a fresh session is required; `/reload-plugins` alone may not pick up the updated hook allowlist depending on the Claude Code CLI version.
...
2. **Editing `hooks.json` without running fleet sync** — `hooks/hooks.json` is the SSoT for hook intent. After editing it, run `bun run ~/.codex/scripts/sync-claude-palantir-mini.ts` or `bun scripts/sync-codex-adapter.ts` from `plugins/palantir-mini` before restarting Codex.
   ...
   reduced 104-command hook surface is visible.
...
6. **Testing hook changes in Claude Code CLI without verifying Codex parity** — supported Codex hook events are adapter-backed, but Claude-only task/team/subagent surfaces remain gaps. Consult `docs/NATIVE_RUNTIME_GAPS.md` before declaring a hook "working cross-runtime."
