...
test("capability-split member RIDs reference real live agents + partition the 8-agent surface", () => {
  const live = liveAgentIds();
  // 2 read-only + 6 mutating = the full 8-agent surface, disjoint.
  expect(READ_ONLY_AGENT_IDS.length).toBe(2);
  expect(MUTATING_AGENT_IDS.length).toBe(6);
  const all = [...READ_ONLY_AGENT_IDS, ...MUTATING_AGENT_IDS];
  expect(new Set(all).size).toBe(8); // disjoint, no overlap
