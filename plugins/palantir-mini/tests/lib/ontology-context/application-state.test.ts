...
//   (1) headline           — real project root returns all 13 sub-fields with ≥10 available
...
// Sub-fields that carry an `available: boolean` flag (per spec §13 sub-fields).
...
describe("composeApplicationState — Phase 3 PR 3.2 composer (sprint-094)", () => {
  test("(1) headline — real project root returns all 13 sub-fields with ≥10 available", async () => {
    ...
    // ≥7 of the 9 availability-bearing sub-fields available.
