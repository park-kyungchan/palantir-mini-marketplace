...
source-author: Lance Martin (with Gabe Cemaj and Michael Cohen)
...
topic: "Scaling Managed Agents — decoupling brain (Claude + harness) from hands (sandbox/tools) and session (event log) so each can be replaced independently across model generations"
---

# Scaling Managed Agents: Decoupling the brain from the hands

> Published Apr 08, 2026 by Lance Martin, Gabe Cemaj, and Michael Cohen, Anthropic Engineering.
> Source: https://www.anthropic.com/engineering/managed-agents
> Cited by `~/.claude/rules/CONTEXT.md §15 Glossary` and `§17 Brain-of-Swarms layer model` as canonical 1차 자료 for the **Brain / Hands / Session** decomposition + the **meta-harness** definition that Managed Agents instantiates.
...
> *(Original article includes architecture diagram showing decoupled brain/hands/session components.)*
...
> *(Original article includes diagram showing brain ↔ session ↔ hands as three interfaces, each replaceable.)*
...
> *(Original article includes diagram showing context query as positional slice.)*
...
> *(Original article includes diagram showing one brain connected to many hands via uniform `execute` interface.)*
...
- This is the canonical Anthropic source for the **Brain / Hands / Session** layer model used in `~/.claude/rules/CONTEXT.md §17 Brain-of-Swarms layer model`.
- Defines **meta-harness** as: *"a system with general interfaces that allow many different harnesses"* — directly cited by CLAUDE.md §Vocabulary clarifying that Managed Agents is a meta-harness, NOT a single harness species.
- Defines **harness (genus)**: *"the loop that calls Claude and routes Claude's tool calls to the relevant infrastructure"* — direct quote underlying the rule 16 v4.0.0 5-species taxonomy.
- Defines **cattle, not pets** principle for sandboxes — referenced in rule 25 v1.0.0 §Post-merge actions discipline.
- Companion to: Justin Young (2025-11-26) `effective-harnesses-2025-11-26.md` + Prithvi Rajasekaran (2026-03-24) `harness-design-2026-03-24.md`. All three together = Anthropic 1차 자료 trio cited by CLAUDE.md §Vocabulary.
- TTFT performance numbers (p50 −60%, p95 −90%) are the headline empirical claim for the decoupling architecture.
