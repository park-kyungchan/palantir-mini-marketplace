---
source-url: https://platform.claude.com/docs/en/about-claude/models/whats-new-claude-4-7
source-author: Anthropic team
source-published: "2026-04-16 (live page; fetched 2026-05-06)"
...
topic: "Platform-side breaking changes for Opus 4.7 — sampling params removed (400 error), budget_tokens removed, thinking off-by-default, 1.0-1.35× tokenizer inflation, task_budgets beta"
---

# What's new in Claude Opus 4.7

> Published 2026-04-16 (live page; fetched 2026-05-06) by Anthropic team.
> Source: https://platform.claude.com/docs/en/about-claude/models/whats-new-claude-4-7
> Cited by Wave 2 sprint-046 Angle A as canonical 1차 자료 on Opus 4.7 Messages API breaking changes (sampling params, thinking budgets, tokenizer, task_budgets beta).
...
> **Note**: These breaking changes apply to the Messages API only. If you use Claude Managed Agents, there are no breaking API changes for Claude Opus 4.7.
...
> **Note**: If your product streams reasoning to users, the new default will appear as a long pause before output begins. Set `"display": "summarized"` to restore visible progress during thinking.
...
- [Task budgets](/docs/en/build-with-claude/task-budgets) — Give Claude an advisory token budget across a full agentic loop.
- [Adaptive thinking](/docs/en/build-with-claude/adaptive-thinking) — The only supported thinking-on mode on Claude Opus 4.7.
- [Effort](/docs/en/build-with-claude/effort#recommended-effort-levels-for-claude-opus-4-7) — Per-level effort guidance for Claude Opus 4.7.
- [Images and vision](/docs/en/build-with-claude/vision) — High-resolution image support and 1:1 coordinate mapping.
- [Migration guide](/docs/en/about-claude/models/migration-guide#migrating-to-claude-opus-4-7) — Step-by-step upgrade instructions.

---

## Local indexing notes

- Cited by Wave 2 research wave (sprint-046 Angle A) for Opus 4.7 Messages API breaking changes.
- Companion to `opus-4-7-introducing-2026-04-16.md` (announcement) and `opus-4-7-postmortem-2026-04-23.md` (April 23 incident).
- Critical breaking-change deltas vs Opus 4.6: (a) sampling params (`temperature`, `top_p`, `top_k`) → 400 error; (b) `thinking: {budget_tokens: N}` → 400 error (use `{type: "adaptive"}`); (c) thinking content omitted from response by default; (d) tokenizer inflation 1.0–1.35×; (e) new `task_budget` beta (header `task-budgets-2026-03-13`).
