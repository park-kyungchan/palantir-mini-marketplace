...
source-author: Janakiram MSV
source-published: 2026-04-18T12:40:00-04:00
fetched-at: 2026-05-06T13:30:00Z
license-note: "External source — read-only mirror for AI-agent research SSoT per ~/.claude/CLAUDE.md §Artifact Layer Policy. Do not redistribute outside ~/.claude/research/. The New Stack carries an editorial paywall on portions of the site; this body was retrieved via stealthy fetch of the publicly-served article URL."
topic: "The New Stack — Anthropic + OpenAI + Google + Microsoft 4-vendor consensus that harness is the product (April 2026); pricing model split + 16-day timing convergence; Sycamore $65M seed (March 30) → Anthropic Managed Agents $0.08/session-hour (April 8) → OpenAI open-source Agents SDK harness (April 15)"
---

# Anthropic, OpenAI, Google, and Microsoft agree that the harness is the product. They disagree on the price.

> Published 2026-04-18 12:40pm by Janakiram MSV, The New Stack.
> Source: https://thenewstack.io/ai-agent-harness-pricing-split/
> Subtitle: "Anthropic, OpenAI, Google and Microsoft split on AI agent harness pricing as Anthropic charges $0.08 per session hour and OpenAI ships open source."
> Cited by `~/.claude/research/harness-engineering-2026/INDEX.md` as the industry-canonical 4-vendor convergence framing for the W2 5-angle research wave (sprint-046, Angle D).

## Article body (verbatim mirror — preserves emphasis, links, and quotes)
...
The question to watch is which business model wins, or whether the market sustains all of them. OpenAI's bet is that a free, open-source, model-native harness drives more model consumption than a paid managed runtime ever would, and that the Bedrock partnership covers enterprises that want hosted. Anthropic is betting on a paid, fully managed version, while Google and Microsoft are betting on packaging priced primitives within a broader platform. Not all of these can be right at the same scale, and the startups watching from outside need to know which way the volume moves before they commit to their differentiation strategy.

## Disclosure note (preserved from source)

> TNS owner Insight Partners is an investor in: Anthropic, E2B, OpenAI, CrewAI.

## Why this article matters for palantir-mini

The article gives palantir-mini three load-bearing facts for sprint-046+ rules + briefings:

1. **4-vendor consensus on category** — Anthropic + OpenAI + Google + Microsoft all agree the harness is the product. palantir-mini's harness taxonomy (rule 16 v4.0.0 §0; CONTEXT.md §15 Glossary) lists 5 species that match: (1) Claude Code CLI, (2) Claude Agent SDK, (3) task-specific harness, (4) Managed Agents internal, (5) palantir-mini-sprint-harness. The 4-vendor convergence justifies the genus-vs-species distinction palantir-mini already enforces.

2. **Pricing-model divergence** — Anthropic ($0.08/session-hour bundled) vs OpenAI (open-source + BYO sandbox) vs Google (priced primitives) vs Microsoft (model + tool metering) is the strategic context for any palantir-mini decision about hosting Brain (model) vs Hands (sandbox) vs Session (event log). palantir-mini's design (BYO Claude Code CLI / BYO sandbox / append-only events.jsonl) maps to OpenAI's open-source posture, not Anthropic's managed-runtime posture.

3. **Build-vs-buy benchmarks** — for internal palantir-mini cost analysis (`~/.claude/plans/` cost docs), the two new benchmarks are: Anthropic Managed Agents at $0.08/session-hour + standard tokens; OpenAI open-source Agents SDK at standard tokens only + BYO infra. Both predate sprint-046 cost work; both are now reference points.

The article also names Martin Fowler as the entity that "canonized" the term in early April 2026 — confirming the cross-ref to `martin-fowler-harness-engineering.md` in this same dir.

## Cross-refs

- `martin-fowler-harness-engineering.md` — the "Martin Fowler canonized it in early April" reference points here.
- `endor-labs-cursor-not-codex-gpt-5-5.md` — empirical evidence that the harness pricing decision matters because harness choice drives outcomes on identical models.
- `~/.claude/research/anthropic/scaling-managed-agents-2026-04-08.md` — Anthropic's own Brain/Hands/Session framing of the Managed Agents launch this article describes.
- `~/.claude/research/openai/` (M2 sibling) — OpenAI's own framing of the April-15 SDK update this article describes.
