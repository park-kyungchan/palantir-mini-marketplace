...
source-author: Ryan Lopopolo (Member of the Technical Staff, OpenAI). Acknowledgements to Victor Zhu and Zach Brock.
...
topic: "OpenAI's own harness engineering framing — 5-month experiment building 1M LOC product with 0 manually-written lines of code; AGENTS.md-as-table-of-contents pattern; agent legibility as goal; layered architecture enforced via custom linters; entropy/garbage-collection cleanup process"
---

# Harness engineering: leveraging Codex in an agent-first world

> Published February 11, 2026 by Ryan Lopopolo, Member of the Technical Staff, OpenAI.
> Source: https://openai.com/index/harness-engineering/
> Cited by: Wave 2 sprint-046 research wave (vendor positioning on harness-as-product thesis; corroborates Anthropic's harness species framing from a different vendor).
...
The pattern: within each business domain (e.g. App Settings), code can only depend "forward" through a fixed set of layers (Types → Config → Repo → Service → Runtime → UI). Cross-cutting concerns (auth, connectors, telemetry, feature flags) enter through a single explicit interface: Providers. Anything else is disallowed and enforced mechanically.
...
- Cited by Wave 2 sprint-046 research wave (vendor positioning on the harness-as-product thesis).
- **Note on date**: published 2026-02-11, NOT 2026-04 as initially estimated. Author is Ryan Lopopolo, not OpenAI institutional voice — this is engineer-written field-report tone, complementary to the corporate-voice `agents-sdk-next-evolution-2026-04-15.md`.
- **Vendor convergence with palantir-mini operating principles**:
  - "Agents are most effective in environments with strict boundaries and predictable structure" ↔ palantir-mini's ontology-first authority chain (rule 01) + sprint contracts (rule 16) + agent file-ownership (rule 07).
  - "Give Codex a map, not a 1,000-page instruction manual" / `AGENTS.md` as table-of-contents ↔ palantir-mini's BROWSE.md (T1 query router) + INDEX.md (structure + provenance) pattern.
  - "Doc-gardening agent" / quality grades / staleness detection ↔ palantir-mini's `pm-research-staleness-audit` (W1.C SSoT-9 manifest-driven) + `rule-bottleneck-watch` hook + `rule_drift_detect` audit.
  - "Golden principles encoded into repository" + recurring cleanup ↔ palantir-mini's rule 25 (auto-merge + cleanup default) + rule 26 (valuable-data standard with auto-grading).
  - "From the agent's point of view, anything it can't access in-context while running effectively doesn't exist" ↔ rule 10 v2.1.0 §propagationDepth (chain reconstruction grounded in events.jsonl alone).
- Companion to Anthropic's Justin Young 2025-11-26 + Prithvi 2026-03-24 + Lance Martin 2026-04-08 trio: cross-vendor evidence that the "harness as product surface" thesis is industry-wide, not Anthropic-specific.
- Quantitative anchor: 3 → 7 engineers; ~1M LOC; ~1,500 PRs in 5 months; 3.5 PRs per engineer per day average. Useful as cost-effectiveness baseline for any palantir-mini-driven harness-engineering pitch.
