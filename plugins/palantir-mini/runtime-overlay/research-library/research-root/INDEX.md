...
> `~/.claude/research/INDEX.md` into
> `runtime-overlay/research-library/research-root/INDEX.md`.
...
```text
current official public docs (palantir-official/)
  ↓
official builder entrypoint (palantir-developers/)
  ↓
legacy curated official mirror (palantir-foundry/)
  ↓
...
|------|------|------------|------------|
| `palantir-official/` | Current generated official Palantir docs SSoT across Foundry, Apollo, Gotham, Defense OSDK, and API-reference manifest rows | 3533 generated body MD + 1441 manifest-only rows + routing | [Official Palantir] generated from sitemap + `__NEXT_DATA__.markdown` |
...
| `palantir-foundry/` | Legacy curated compatibility mirror for older Foundry/AIP/Apollo citations and pre-2026-05-13 comparison | ~231 MD including routers/canonical additions | [Official Palantir] legacy mirror |
...
- Keep `palantir-official/` generated. Refresh by rerunning `~/.claude/plugins/palantir-mini/scripts/import-palantir-official-docs.ts`, then `audit-palantir-research-overlap.ts`; do not patch generated page bodies.
- Keep `palantir-developers/` thin; it should reduce search cost, not duplicate verbatim docs.
- `palantir-foundry/` is legacy compatibility. Do not add active official-doc authority there unless a later migration explicitly archives/replaces the old mirror.
...
## Official Docs Umbrella 2026-05-13

User directive 2026-05-13: use official `www.palantir.com/docs` as the SSoT under `~/.claude/research/`, keep `BROWSE.md` / `INDEX.md` as precise routers, and validate overlap/conflicts with existing research down to content units.

Generated artifacts:

- `palantir-official/` — current official docs umbrella generated from `https://www.palantir.com/docs/sitemap.xml` and page `__NEXT_DATA__.markdown`.
- `palantir-official/_manifest.json` — 4974 sitemap rows, 3533 markdown body files, 1441 manifest-only rows, 0 failed rows.
- `palantir-official/_crawl-audit.json` — fetch/provenance/filter audit with Scrapling seed-check evidence.
- `~/.claude/plans/2026-05-13-palantir-official-import-report.md` — import closeout.
- `~/.claude/plans/2026-05-13-palantir-research-overlap-audit.{md,json}` — URL/content/risk-term overlap report versus `palantir-foundry/`.

Overlap result:

- Legacy rows checked: 212.
- Exact or near official matches: 211.
- Legacy-only rows: 1 (`agent-studio` overview route; current docs use `chatbot-studio` route).
- Changed matched rows: 133, mainly reflecting upstream route/body drift since the 2026-04-20 curated mirror.
- Duplicate legacy URL groups: 9.

Authority update:

- Current official facts: `palantir-official/`.
- Legacy citation compatibility: `palantir-foundry/`.
- Internal synthesis: `palantir-vision/`.
...
- `AskUserQuestion` as the required loop for semantic ambiguity closure;
