/**
 * palantir-mini SELF-ONTOLOGY — Agent as a registered ObjectType + its 15 instances
 ...
 * Agent identity. This file declares the type and seeds the 15 agents — the snapshot
 * OWNS the seed (it is the authority), so it does NOT import the agents tree uphill. The
 * paired registration test cross-checks these 15 names against the LIVE `agents/`
 * directory so the self-model fails loud if pm's agent surface drifts (an agent
 * added/removed without updating this seed).
 *
 * Count provenance (LIVE-verified): `agents/` holds EXACTLY 15 `*.md` declarations.
 ...
 * Write/Edit/NotebookEdit, else "mutating"). Richer per-agent metadata (workflowFamily,
 * tool bindings, outputContractExempt) lives in each `.md`, not duplicated in the seed.
 *
...
export const AGENT_OBJECT_TYPE: ObjectTypeDeclaration = {
  ...
  properties: [
    ...
    { name: "tier", type: '"opus" | "sonnet" | "haiku"' },
...
/**
 * The 15 Agent instances — pm's LIVE subagent surface, in `agents/` filename order.
...
export const AGENT_INSTANCES: readonly AgentInstance[] = [
  { agentId: "agent-author", tier: "opus", mutationCapability: "mutating" },
  { agentId: "code-grader", tier: "sonnet", mutationCapability: "mutating" },
  { agentId: "docs-researcher", tier: "opus", mutationCapability: "mutating" },
  { agentId: "eval-judge", tier: "opus", mutationCapability: "read-only" },
  { agentId: "hook-builder", tier: "sonnet", mutationCapability: "mutating" },
  { agentId: "implementer", tier: "sonnet", mutationCapability: "mutating" },
  { agentId: "lead-orchestrator", tier: "opus", mutationCapability: "mutating" },
  { agentId: "model-grader", tier: "sonnet", mutationCapability: "mutating" },
  ...
  { agentId: "scrapling-fetcher", tier: "haiku", mutationCapability: "read-only" },
  { agentId: "verifier-adversarial", tier: "opus", mutationCapability: "read-only" },
  { agentId: "verifier-correctness", tier: "opus", mutationCapability: "read-only" },
];

// Register the Agent ObjectType (the type). The 15 instances above are data the
