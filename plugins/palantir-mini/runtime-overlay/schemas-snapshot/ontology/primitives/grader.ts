 ...
 *
 * Alias-wrapper linking the split grader primitives (grading-criterion.ts,
 * grader-effort.ts) into a unified runtime-side dispatch identity. The split
 * is intentional (composable); this wrapper adds the canonical `GraderRid`
 * brand + `GraderDeclaration` composite that links the existing primitives by
 * reference.
 *
 * `GraderDeclaration` is the dispatch-time identity: "which criterion does
 * this grader serve, at what effort tier, in which domain?". The existing
 * primitives retain their independent semantics — this wrapper does NOT
 * collapse them.
 *
 ...
 *
 * D/L/A domain: DATA (dispatch identity record — composites existing primitives)
...
export interface GraderDeclaration {
  ...
  /** Grader domain identifier (evaluator-type slug). */
