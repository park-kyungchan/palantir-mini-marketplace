...
### Added — second-brain memory-fold governed event types (`ontology/lineage/event-types.ts`)

- `EVENT_TYPE_NAMES` + `EVENT_TYPE_REGISTRY` gain `resolution_verdict` and `memory_fold_committed` (both `primaryDomain: "learn"`), the 85th + 86th event discriminators (was 84). These are the two GOVERNED Layer-1 events the second-brain session-end memory fold emits via the gated `emit_event` MCP path (rule 27, NOT raw `appendEventAtomic`): `resolution_verdict` records the entity-resolution verdict (`ADD`/`UPDATE`/`DELETE`/`NONE`) per Layer-2 graph mutation (the log = audit trail of every graph mutation, Mem0 imports #2), and `memory_fold_committed` records the committed Layer-2 `graph.json` projection (node/edge counts) for a session. The self-Ontology seed drift guard tracks both: `ontology/self/event-envelope.objecttype.ts` `EVENT_ENVELOPE_INSTANCES` gains the two members (84 → 86) and `tests/ontology/self/event-envelope-registration.test.ts` `EXPECTED_EVENT_ENVELOPE_COUNT` advances 84 → 86. Paired non-snapshot edits land the runtime envelopes (`lib/event-log/types.ts` `ResolutionVerdictEnvelope` + `MemoryFoldCommittedEnvelope` + union members + `isResolutionVerdict` / `isMemoryFoldCommitted` guards + optional `EventSnapshot` counter fields, and `lib/event-log/read/fold-snapshot.ts` two counting `case` branches) and the grader C1 LEARN-map (`hooks/value-grade-assigner.ts` `learnTypes` Set) so both events are LEARN-mapped on the diagnostic C1 axis. NOTE on grade: the authoritative grader is `autoGradeEnvelope` (`bridge/handlers/emit-event.ts`), which grades on FIELD PRESENCE only and does NOT consult `learnTypes` (that Set lives in `computeAxes14`, whose `axesScored` output feeds only the diagnostic instrumentation meta-event, with zero effect on `assignedTier` — `effectiveGrade = grade` at `value-grade-assigner.ts`). A fully-formed envelope of either type therefore grades ≥ T1 (emittable; above the T0 reject floor); it reaches T3 only when the EMITTER supplies the memory/lineage axis (`withWhat.memoryLayers` + a lineage ref or hypothesis) and the refinement axis (`withWhat.refinementTarget` or `payload.failureCategory`) at emit time — no type registration can force T3. The session-end fold emitter is responsible for populating those axis fields when it wants T3+ retention. Additive + backward-compatible — no consumer that switches on the existing discriminators breaks.

### Added — additive per-row `envelopeRev` field + on-read upcaster path (`ontology/primitives/event-envelope.ts` + `lib/event-log/types.ts` + `lib/event-log/upcasters/`)

- `EventEnvelopeBase` gains an additive optional `envelopeRev?: number` in LOCKSTEP across the canonical schema primitive (`ontology/primitives/event-envelope.ts`) and its runtime mirror (`lib/event-log/types.ts`). It is the per-row schema-revision tag keyed on by the new on-read upcaster `lib/event-log/upcasters/upcastEnvelope(raw): EventEnvelope` (identity transform for `undefined` / current rev; a small registry for future revs). Deliberately NOT named `schemaVersion` — that collides with the existing module-level string const `EVENT_ENVELOPE_SCHEMA_VERSION` (`"events-jsonl/event-envelope/v1"`), which versions the whole envelope schema rather than a row. Absent on every legacy row (⇒ rev 0 / current), so the change is additive + backward-compatible: an upcaster is an on-read transform, never an in-place migration.
