...
The plugin carries a portable copy of the external research router and current
Palantir official docs:

- `research-root/BROWSE.md` and `research-root/INDEX.md` are vendored from
  `~/.claude/research/{BROWSE,INDEX}.md`.
- `palantir-official/` is vendored from
  `~/.claude/research/palantir-official/`.

Path mapping for plugin-only runtimes: when vendored docs say
`~/.claude/research/<topic>`, resolve it as
`runtime-overlay/research-library/<topic>`. External research remains the
refresh/provenance source; the plugin snapshot is the portable runtime source.
...
Do not inject whole research or schema directories. Product facts come from
`palantir-developers/` and `palantir-foundry/`; `palantir-vision/` is synthesis
only until promoted downstream.
...
| What do verbatim official Palantir Foundry docs say? | `palantir-foundry/BROWSE.md` |
...
| What did the 2026-05-09 Scrapling deep dive find about Palantir Community Ontology Engineering failures ambiguity closure, and Lead intent-to-Digital-Twin gaps? | `INDEX.md` §2026-05-09 Lead Intent to Digital Twin deep-dive update -> `~/.claude/plans/2026-05-09-lead-intent-to-digital-twin-gap-analysis.md` |
...
1. `palantir-developers/BROWSE.md` -> `palantir-developers/build-with-aip.md` for the official builder entrypoint.
2. `palantir-foundry/architecture/BROWSE.md` -> `architecture-center-aip-architecture.md` and `ontology-overview.md` for the official architecture baseline.
3. `palantir-foundry/aip/BROWSE.md` -> `ai-fde-overview-and-modes-skills-2026-03-12.md`, `aip-evals-overview-and-ontology-edits-2026-04-14.md`, and `workflow-lineage-and-aip-observability-2026-03-03.md`.
4. `palantir-foundry/dev-toolchain/BROWSE.md` -> `palantir-mcp-and-ontology-mcp-2026-03-26.md` for builder-vs-consumer MCP boundaries.
5. `palantir-foundry/ontology/BROWSE.md` -> `global-branching-overview-2026-05-05.md` plus the exact Action Type pages only if action semantics are in scope.
6. `palantir-vision/architecture-gap/BROWSE.md` -> `semantic-intent-gate-for-ontology-engineering.md` for local interpretation.
7. `~/.claude/plans/2026-05-09-lead-intent-to-digital-twin-gap-analysis.md` for the Claude-facing proposal, community failure catalog loop requirements, and final-review permission for major palantir-mini rewrites.
...
- `palantir-foundry/` — **official fact layer**. Verbatim official Palantir docs fetched from `palantir.com/docs`. Use when exact wording or product-surface detail matters.
...
- Conflict rule:
  - When `palantir-developers/` and `palantir-foundry/` disagree on an exact fact, `palantir-foundry/` wins.
...
- Start with `palantir-developers/` for builder questions, then descend into `palantir-foundry/` only when a detail remains unresolved.
- Start with `palantir-foundry/` directly only when the user asks for exact official wording or product-surface mechanics.
...
- `palantir-foundry/` files are verbatim or router/index docs around verbatim content; do not hand-edit scraped page bodies.
