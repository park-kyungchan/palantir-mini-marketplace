...
---
# Core Invariants
16 active rules (sprint-060 W1.11). Detail: `pm_rule_query({ byId: NN })`. Map: `CONTEXT.md`.
...
- **Lead Protocol** (12 v3.9.0): model pinned by role; 3-tier fallback chain; lazy-spawn + auto-shutdown; task granularity DELETE/ADD/KEEP/VERIFY + 1-file + ≤15K; Quick Sprint auto-wrap; briefing template 5-section; **Pre-delegation framework + Lead self-test directive + Lead-direct edit threshold + Subagent decision audit invariant + Complex-task EnterPlanMode protocol + Harness Engineering context awareness + Pre-delegation hard gate + Task context budget hard gate**.
- **Sprint-harness species** (16 v4.1.0): palantir-mini-sprint-harness B2 hard default-on; one of 5 harness species; Brain dispatches across all; generator-tier agents use `isolation: "worktree"` (cattle-not-pets).
