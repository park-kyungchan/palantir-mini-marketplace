...
description: "Systematic root-cause debugging. Four phases — investigate, analyze, hypothesize, implement. Iron Law — no fixes without root cause."
...
Call `mcp__palantir-mini__pm_preamble` once at session start to load project-specific
...
5. **Check investigation history.** Search prior learnings for investigations on
   the same files. Call `mcp__palantir-mini__pm_learn_query` with
   `{ query: "<affected-file or component name>", limit: 20 }` and look for entries
...
6. **Check BackwardProp trace.** If the bug involves ontology drift, contract
   ...
   `mcp__palantir-mini__replay_lineage` with a 5-dim filter keyed on the affected
