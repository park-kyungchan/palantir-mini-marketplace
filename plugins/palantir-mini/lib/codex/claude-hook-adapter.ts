...
function resolveOptions(options: CodexAdapterOptions = {}): ResolvedOptions {
  ...
  const pluginRoot =
    ...
    options.env?.CLAUDE_PLUGIN_ROOT ??
...
async function emitCodexAdapterTimeoutObserved(
  ...
  try {
    await emit({
      ...
      toolName: "codex-claude-hook-adapter",
      cwd: projectRoot,
...
async function emitCodexAdapterCapabilityMismatch(
  ...
  try {
    await emit({
      ...
      reasoning:
        `Codex adapter received Claude-only event=${eventName}; emitted the documented fallback fact instead of claiming runtime parity.`,
    });
  } catch {
    // Best-effort diagnostics must never change hook behavior.
  }
...
function claudePalantirToolName(toolName: string): string | undefined {
...
export function adaptPayloadForClaude(payload: JsonObject): JsonObject {
  ...
  const claudeMcpName = claudePalantirToolName(toolName);
  if (claudeMcpName) {
    next.tool_name = claudeMcpName;
    return next;
  }
...
export async function runCommandHook(
  ...
  const hookPayload = adaptPayloadForClaude(payload);
  const proc = Bun.spawn(["bash", "-lc", command], {
    ...
    env: {
      ...
      CLAUDE_PLUGIN_ROOT: resolved.pluginRoot,
