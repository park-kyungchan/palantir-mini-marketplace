// Permanent regression for src/adapters/codex/flat-schema.ts (ledger row
// A620, validation-contract item 2: "Flat-schema validator: every public
// MCP input schema is flat, no anyOf/oneOf/allOf/not (test evidence against
// the exact schemas shipped)"). Colocated (not under tests/adapters/**)
// because this row's exact write set does not include tests/adapters/**
...
describe("isFlatMcpInputSchema: every schema this plugin actually ships is flat", () => {
  test("CODEX_BINDING carries at least one tool (non-trivial coverage)", () => {
    expect(CODEX_BINDING.tools.length).toBeGreaterThan(0);
  });
