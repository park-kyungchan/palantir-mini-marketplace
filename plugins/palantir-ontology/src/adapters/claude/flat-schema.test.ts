// Permanent regression for src/adapters/claude/flat-schema.ts (ledger row
// A630, validation-contract item 2 equivalent: "every public MCP input
// schema this row ships is flat, no anyOf/oneOf/allOf/not (test evidence
...
// it to A610 only), mirroring src/adapters/codex/flat-schema.test.ts.
...
describe("isFlatMcpInputSchema: every schema this plugin actually ships is flat", () => {
  test("CLAUDE_BINDING carries at least one tool (non-trivial coverage)", () => {
    expect(CLAUDE_BINDING.tools.length).toBeGreaterThan(0);
  });
